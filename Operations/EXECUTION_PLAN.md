...
**Status:** 🟢 Active — the new expansion master plan triplet and the full MATHS prompt inventory are now in place. `E0A` through `E6A` are complete, `E6B` is the active next act, and `E6C` through `E9E` remain planned and blocked by predecessor work.
**Current next act:** `E6B` — build the HR department capability pack on top of the new shared department-pack runtime.
