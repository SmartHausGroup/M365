...
def _graph_token(prefer_delegated: bool = False) -> Optional[str]:
    ...
    if tenant_id and client_id and cert_path and os.path.exists(cert_path):
        try:
            ...
            cred = ClientCertificateCredential(tenant_id=tenant_id, client_id=client_id, certificate_path=cert_path)
...
async def _graph_request(method: str, url: str, correlation_id: str, json_body: Dict[str, Any] | None = None, params: Dict[str, Any] | None = None) -> Dict[str, Any] | None:
    token = _graph_token()
    if not token:
        if _stub_mode():
...
async def _graph_request_raw(method: str, url: str, correlation_id: str, params: Dict[str, Any] | None = None) -> str:
...
async def m365_users_read(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    data = await _graph_request("GET", url, correlation_id, params={"$select": "id,displayName,userPrincipalName,accountEnabled,jobTitle,department"})
    return {"user": data}
...
async def m365_users_list(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"users": [
            {"id": "00000000-0000-0000-0000-000000000000", "displayName": "Stub User", "userPrincipalName": "stub.user@smarthaus.ai", "accountEnabled": True},
        ], "count": 1}
    select = "id,displayName,userPrincipalName,accountEnabled,jobTitle,department,mail"
    url = f"{GRAPH_BASE}/users"
    data = await _graph_request("GET", url, correlation_id, params={"$select": select, "$top": str(params.get("limit", 100))})
...
async def _get_subscribed_skus(correlation_id: str) -> List[Dict[str, Any]]:
    if _stub_mode():
        return [
            {"skuId": "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa", "skuPartNumber": "ENTERPRISEPACK", "prepaidUnits": {"enabled": 100}, "consumedUnits": 10, "availableUnits": 90},
            {"skuId": "bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb", "skuPartNumber": "M365_E5", "prepaidUnits": {"enabled": 50}, "consumedUnits": 45, "availableUnits": 5},
        ]
...
def _resolve_requested_licenses(requested: List[str], by_id: Dict[str, Any], by_part: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], List[Dict[str, str]]]:
...
async def m365_licenses_assign(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    for sku in resolved:
        ...
        if sku_id in assigned:
            skipped.append({"skuId": sku["skuId"], "skuPartNumber": sku["skuPartNumber"], "reason": "already_assigned"})
            continue
        if sku.get("availableUnits", 0) <= 0:
            skipped.append({"skuId": sku["skuId"], "skuPartNumber": sku["skuPartNumber"], "reason": "insufficient_seats"})
            continue
...
async def outreach_email_send_individual(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if not recipients:
        return {"email": "rejected", "error": "missing_recipients", "note": "Param 'to' is required"}
    ...
    sender = params.get("from") or os.getenv("OUTREACH_SENDER") or os.getenv("DEFAULT_SENDER") or os.getenv("MAIL_SENDER")
    ...
    if not sender:
        ...
        return {"email": "rejected", "error": "sender_missing", "note": "Provide 'from' or set OUTREACH_SENDER"}
    ...
    await _graph_request("POST", url, correlation_id, json_body={"message": msg, "saveToSentItems": save_to_sent})
...
async def mail_list(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"messages": [
            {"id": "msg-stub-001", "subject": "Stub email", "from": {"emailAddress": {"address": "sender@example.com"}}, "receivedDateTime": "2024-01-15T10:00:00Z", "isRead": True},
        ], "count": 1}
    url = f"{GRAPH_BASE}/users/{user_id}/messages"
    data = await _graph_request("GET", url, correlation_id, params={
...
async def mail_read(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"message": {"id": message_id, "subject": "Stub email", "body": {"contentType": "Text", "content": "Stub body"}, "from": {"emailAddress": {"address": "stub@example.com"}}}}
...
async def mail_move(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    data = await _graph_request("POST", url, correlation_id, json_body={"destinationId": destination_id})
    return {"moved": True, "messageId": message_id, "destinationId": destination_id, "message": data}
...
async def mail_folders(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"folders": [
            ...
            {"id": "sentitems", "displayName": "Sent Items", "totalItemCount": 100, "unreadItemCount": 0},
        ]}
...
async def mailbox_settings(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    user_id = params.get("userId") or params.get("userPrincipalName") or "me"
    if _stub_mode():
        return {"settings": {"automaticRepliesSetting": {"status": "disabled"}, "language": {"locale": "en-US"}, "timeZone": "UTC"}}
...
async def teams_list(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"teams": [
            {"id": "team-stub-001", "displayName": "Stub Team", "description": "A stub team", "mail": "stubteam@smarthaus.ai"},
        ], "count": 1}
...
async def teams_get(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"team": {"id": team_id, "displayName": "Stub Team", "description": "A stub team", "isArchived": False}}
...
async def teams_create(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        tid = str(_uuid.uuid4())
        return {"team": {"id": tid, "displayName": display_name, "description": description}, "status": "created"}
    body: Dict[str, Any] = {
...
async def channels_create(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"channel": {"id": str(_uuid.uuid4()), "displayName": display_name, "membershipType": membership_type}, "status": "created"}
    url = f"{GRAPH_BASE}/teams/{team_id}/channels"
    body: Dict[str, Any] = {
        "displayName": display_name,
        "description": description,
...
async def chat_list(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if user_id:
        ...
        raise GraphAPIError(
            400, "no_user_context",
            "userId is required for app-only auth (cannot use /me)."
        )
...
async def chat_send(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if not sender or not recipient:
        raise GraphAPIError(400, "missing_params", "'from' (sender) and 'to' (recipient) are required")

    if _stub_mode():
        return {"chat": {"id": "chat-stub-001"}, "message": {"id": "msg-stub-001"}, "status": "sent"}
...
async def create_workspace(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    for ch_name in (params.get("channels") or ["General"]):
        if ch_name.lower() == "general":
            ...
            continue
        ch_result = await channels_create({"teamId": team_id, "displayName": ch_name}, correlation_id)
...
async def groups_list(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"groups": [
            {"id": "grp-stub-001", "displayName": "Stub Group", "mail": "stubgroup@smarthaus.ai", "groupTypes": ["Unified"], "securityEnabled": False},
        ], "count": 1}
    url = f"{GRAPH_BASE}/groups"
    data = await _graph_request("GET", url, correlation_id, params={
...
async def groups_create(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    mail_nickname = params.get("mailNickname") or display_name.replace(" ", "").lower() if display_name else "group"
    security_enabled = bool(params.get("securityEnabled", False))
    group_types = params.get("groupTypes") or ["Unified"]
    if _stub_mode():
        import uuid as _uuid
        return {"group": {"id": str(_uuid.uuid4()), "displayName": display_name, "mailNickname": mail_nickname, "groupTypes": group_types, "securityEnabled": security_enabled}, "status": "created"}
    url = f"{GRAPH_BASE}/groups"
    body: Dict[str, Any] = {
        "displayName": display_name,
    ...
    if params.get("owners"):
        body["owners@odata.bind"] = [f"https://graph.microsoft.com/v1.0/users/{o}" for o in params["owners"]]
    if params.get("members"):
        body["members@odata.bind"] = [f"https://graph.microsoft.com/v1.0/users/{m}" for m in params["members"]]
    data = await _graph_request("POST", url, correlation_id, json_body=body)
...
async def groups_list_members(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"members": [
            {"id": "usr-stub-001", "displayName": "Stub User", "userPrincipalName": "stub@smarthaus.ai"},
        ], "count": 1}
...
async def calendar_list(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"events": [
            {"id": "evt-stub-001", "subject": "Stub Meeting", "start": {"dateTime": "2024-01-20T10:00:00", "timeZone": "UTC"}, "end": {"dateTime": "2024-01-20T11:00:00", "timeZone": "UTC"}, "organizer": {"emailAddress": {"address": "organizer@smarthaus.ai"}}},
        ], "count": 1}
...
async def calendar_create(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        import uuid as _uuid
        return {"event": {"id": str(_uuid.uuid4()), "subject": params.get("subject", "Stub Event"), "status": "created"}, "status": "created"}
    url = f"{GRAPH_BASE}/users/{user_id}/events"
    body: Dict[str, Any] = {}
    for field in ("subject", "body", "start", "end", "location", "attendees", "isOnlineMeeting", "recurrence"):
        if field in params:
...
async def calendar_availability(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"schedules": [{"scheduleId": s, "availabilityView": "0000000000", "scheduleItems": []} for s in schedules]}
    url = f"{GRAPH_BASE}/users/{user_id}/calendar/getSchedule"
    body: Dict[str, Any] = {
        "schedules": schedules,
        "startTime": start_time if isinstance(start_time, dict) else {"dateTime": start_time, "timeZone": "UTC"},
        "endTime": end_time if isinstance(end_time, dict) else {"dateTime": end_time, "timeZone": "UTC"},
...
async def sites_list(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"sites": [
            {"id": "site-stub-001", "displayName": "Stub Site", "webUrl": "https://smarthaus.sharepoint.com/sites/stub"},
        ], "count": 1}
...
async def sites_get(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"site": {"id": site_id, "displayName": "Stub Site", "webUrl": "https://smarthaus.sharepoint.com/sites/stub", "description": ""}}
...
async def sites_root(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    """Get the root site."""
    if _stub_mode():
        return {"site": {"id": "root-stub", "displayName": "Root Site", "webUrl": "https://smarthaus.sharepoint.com"}}
...
async def lists_list(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"lists": [
            {"id": "list-stub-001", "displayName": "Stub List", "list": {"template": "genericList"}},
        ], "count": 1}
...
async def files_list(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"files": [
            {"id": "file-stub-001", "name": "stub-document.docx", "size": 12345, "webUrl": "https://smarthaus.sharepoint.com/stub-document.docx"},
        ], "count": 1}
    if drive_id:
...
async def files_create_folder(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        import uuid as _uuid
        return {"folder": {"id": str(_uuid.uuid4()), "name": folder_name, "folder": {"childCount": 0}}, "status": "created"}
...
async def files_upload(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if not local_path:
        raise GraphAPIError(400, "missing_param", "filePath is required — the local path to the file to upload.")
    if not remote_path:
        raise GraphAPIError(400, "missing_param", "remotePath is required — the destination path in the drive (e.g. 'Folder/file.docx').")
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if resp.status_code >= 400:
                try:
                    body = resp.json()
                    err = body.get("error", {})
                    raise GraphAPIError(resp.status_code, err.get("code"), err.get("message", resp.text))
                except Exception:
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        while offset < file_size:
            ...
            if resp.status_code >= 400:
                raise GraphAPIError(resp.status_code, "chunk_upload_failed",
...
async def files_share(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"link": {"webUrl": f"https://smarthaus.sharepoint.com/share/{item_id}", "type": link_type, "scope": scope}}
...
async def drives_list(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    site_id = params.get("siteId")
    if _stub_mode():
        return {"drives": [{"id": "drive-stub-001", "name": "OneDrive", "driveType": "personal"}], "count": 1}
    if group_id:
...
async def security_alerts(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"alerts": [
            {"id": "alert-stub-001", "title": "Stub Alert", "severity": "medium", "status": "new", "createdDateTime": "2024-01-15T10:00:00Z"},
        ], "count": 1}
...
async def security_alert_get(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"alert": {"id": alert_id, "title": "Stub Alert", "severity": "medium", "status": "new"}}
...
async def security_incidents(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"incidents": [
            {"id": "inc-stub-001", "displayName": "Stub Incident", "severity": "high", "status": "active", "createdDateTime": "2024-01-15T10:00:00Z"},
        ], "count": 1}
...
async def security_incident_get(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"incident": {"id": incident_id, "displayName": "Stub Incident", "severity": "high", "status": "active"}}
...
async def security_secure_score(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"secureScore": {"id": "score-stub", "currentScore": 72.5, "maxScore": 100, "createdDateTime": "2024-01-15T10:00:00Z"}}
...
async def audit_directory(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"audits": [
            {"id": "audit-stub-001", "activityDisplayName": "Add user", "activityDateTime": "2024-01-15T10:00:00Z", "result": "success"},
        ], "count": 1}
...
async def audit_signins(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"signIns": [
            {"id": "signin-stub-001", "userPrincipalName": "stub@smarthaus.ai", "createdDateTime": "2024-01-15T10:00:00Z", "status": {"errorCode": 0}},
        ], "count": 1}
...
async def audit_provisioning(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"logs": [
            {"id": "prov-stub-001", "activityDateTime": "2024-01-15T10:00:00Z", "provisioningAction": "create", "statusInfo": {"status": "success"}},
        ], "count": 1}
...
async def health_overview(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"services": [
            {"id": "Exchange Online", "service": "Exchange Online", "status": "serviceOperational", "isActive": True},
            {"id": "Microsoft Teams", "service": "Microsoft Teams", "status": "serviceOperational", "isActive": True},
        ]}
...
async def ca_policy_create(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        import uuid as _uuid
        return {"policy": {"id": str(_uuid.uuid4()), "displayName": params.get("displayName", "Stub Policy"), "state": params.get("state", "disabled")}, "status": "created"}
    url = f"{GRAPH_BASE}/identity/conditionalAccess/policies"
    body: Dict[str, Any] = {}
...
async def ca_named_locations(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"locations": [
            {"id": "loc-stub-001", "displayName": "Trusted Network", "@odata.type": "#microsoft.graph.ipNamedLocation"},
        ], "count": 1}
...
async def directory_roles(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"roles": [
            {"id": "role-stub-001", "displayName": "Global Administrator", "description": "Can manage all aspects of Azure AD"},
        ], "count": 1}
...
async def directory_role_members(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"members": [
            {"id": "usr-stub-001", "displayName": "Stub Admin", "userPrincipalName": "admin@smarthaus.ai"},
        ], "count": 1}
...
async def directory_org(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"organization": {"id": "org-stub", "displayName": "Smarthaus", "verifiedDomains": [{"name": "smarthaus.ai"}]}}
...
async def apps_list(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"applications": [
            {"id": "app-stub-001", "displayName": "Stub App", "appId": "00000000-0000-0000-0000-000000000000"},
        ], "count": 1}
...
async def service_principals_list(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"servicePrincipals": [
            {"id": "sp-stub-001", "displayName": "Stub SP", "appId": "00000000-0000-0000-0000-000000000000"},
        ], "count": 1}
...
async def reports_users_active(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"report": [{"userPrincipalName": "stub@smarthaus.ai", "hasExchangeLicense": "True", "hasTeamsLicense": "True"}], "period": period}
...
async def reports_email_activity(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    period = params.get("period", "D7")
    if _stub_mode():
        return {"report": [{"userPrincipalName": "stub@smarthaus.ai", "sendCount": "10", "receiveCount": "25"}], "period": period}
...
async def reports_teams_activity(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    period = params.get("period", "D7")
    if _stub_mode():
        return {"report": [{"userPrincipalName": "stub@smarthaus.ai", "teamChatMessageCount": "5"}], "period": period}
...
async def reports_sharepoint_usage(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    period = params.get("period", "D7")
    if _stub_mode():
        return {"report": [{"siteUrl": "https://smarthaus.sharepoint.com/sites/stub", "fileCount": "100"}], "period": period}
...
async def reports_onedrive_usage(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    period = params.get("period", "D7")
    if _stub_mode():
        return {"report": [{"ownerPrincipalName": "stub@smarthaus.ai", "storageUsedInBytes": "500000"}], "period": period}
...
async def devices_list(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"devices": [
            {"id": "dev-stub-001", "deviceName": "STUB-PC", "operatingSystem": "Windows", "osVersion": "10.0.19045", "complianceState": "compliant", "userPrincipalName": "stub@smarthaus.ai"},
        ], "count": 1}
...
async def devices_get(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"device": {"id": device_id, "deviceName": "STUB-PC", "operatingSystem": "Windows", "complianceState": "compliant"}}
...
async def devices_compliance(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        return {"summaries": [
            {"settingName": "Require encryption", "compliantDeviceCount": 45, "nonCompliantDeviceCount": 5},
        ]}
...
async def website_deployment_preview(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    return {
        "status": "queued",
        "provider": "vercel",
        "note": ("Vercel token not configured; dry-run" if not token else "Request would be sent to Vercel"),
    }
...
async def list_projects(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if _stub_mode():
        ...
        projects = [p for p in items if (status_filter == "all" or p.get("status") == status_filter)]
        return {"projects": projects[: max(1, min(limit, 100))], "total": len(projects), "status": status_filter}
...
def _find_tenant_yaml(tenant_slug: str | None = None) -> str | None:
    ...
    if repos_root:
        search_paths.append(os.path.join(repos_root, "SMARTHAUS_MCPSERVER_core", "tenants", f"{slug}.yaml"))

    # Relative from this file: ops_adapter/ -> src/ -> M365/ -> ../SMARTHAUS_MCPSERVER_core/tenants/
    this_dir = os.path.dirname(__file__)
    search_paths.append(
        os.path.normpath(os.path.join(this_dir, "..", "..", "..", "SMARTHAUS_MCPSERVER_core", "tenants", f"{slug}.yaml"))
    )
...
async def admin_list_users(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if not path:
        raise GraphAPIError(404, "tenant_not_found", f"Tenant config '{tenant_slug}.yaml' not found")
...
async def admin_set_user_tier(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    prev_tier = data["permission_tiers"]["users"].get(email, data["permission_tiers"].get("default_tier", "standard_user"))
...
async def admin_audit_log(params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    """Return recent UCP admin operations from the audit trail.

    Note: Full audit logging will be implemented with the UCP event bus.
    Currently returns the current state snapshot for inspection.
    """
    tenant_slug = params.get("tenant") or os.getenv("UCP_TENANT", "smarthaus")
    path = _find_tenant_yaml(tenant_slug)

    result = {"tenant": tenant_slug, "status": "snapshot_mode"}

    if path:
        with open(path, encoding="utf-8") as f:
            data = yaml.safe_load(f) or {}
        pt = data.get("permission_tiers", {})
        result["current_assignments"] = pt.get("users", {})
        result["default_tier"] = pt.get("default_tier", "standard_user")
        result["user_count"] = len(pt.get("users", {}))
        result["config_file_mtime"] = os.path.getmtime(path)
    else:
        result["error"] = "tenant_config_not_found"

    result["note"] = "Full event-sourced audit log will be available in UCP v2. Currently returning state snapshot."
    return result
...
async def execute(agent: str, action: str, params: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
    ...
    if agent == "project-coordination-agent":
        ...
        if action == "task.assign":
            return {"assigned": True, "task_id": params.get("task_id"), "assignee": params.get("assignee")}
        if action == "deadline.track":
            return {"tracked": True, "deadline": params.get("deadline"), "status": "on_track"}
        if action == "status.update":
            return {"updated": True, "project_id": params.get("project_id"), "status": params.get("status")}
        if action == "report.generate":
            return {"generated": True, "report_type": "weekly", "download_url": "/reports/weekly.pdf"}
    ...
    if agent == "client-relationship-agent":
        ...
        if action == "engagement.plan":
            return {"plan_created": True, "client_id": params.get("client_id"), "actions": ["send newsletter", "schedule call"]}
    ...
    if agent == "recruitment-assistance-agent":
        if action == "candidate.screen":
            return {"screened": True, "candidate_id": params.get("candidate_id"), "recommendation": "interview"}
        if action == "interview.schedule":
            return {"scheduled": True, "candidate_id": params.get("candidate_id"), "interviewer": params.get("interviewer")}
