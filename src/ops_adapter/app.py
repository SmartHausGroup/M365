...
def create_app() -> FastAPI:
    ...
    def _resolve_task_queue_target(agent_target: str) -> tuple[str, dict[str, Any]]:
        canonical_agent, persona = resolve_persona_target(agent_target, personas)
    ...
    def _resolve_task_actor(request: Request) -> str:
    ...
    async def queue_persona_task(
        ...
        canonical_agent, persona = _resolve_task_queue_target(agent_target)
        payload = dict(body)
        payload.setdefault("created_by", _resolve_task_actor(request))
        try:
    ...
    async def set_persona_task_state(
        ...
        canonical_agent, persona = _resolve_task_queue_target(agent_target)
        payload = dict(body)
        payload.setdefault("updated_by", _resolve_task_actor(request))
        try:
