#!/usr/bin/env python3
"""
SmartHaus Repository Analyzer
Automatically analyzes git repositories and generates project plans
based on actual development history
"""
...
app = typer.Typer(add_completion=False, help="Repository Analysis and Project Plan Generation")
...
def analyze_git_repository(repo_path: Path, months_back: int) -> Dict[str, Any]:
    ...
    try:
        ...
        project_plan = {
            "project": {
                "name": repo_name,
                "description": f"Project plan derived from actual git commit history",
                "repository": repo_url,
...
def analyze_development_phases(commits: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if is_new_phase(commit, commits[max(0, i-3):i]):
            if current_phase:
                ...
                phases[current_phase]['duration'] = calculate_duration(
                    phases[current_phase]['start_date'], 
                    phase_start
                )
...
def is_new_phase(commit: Dict[str, Any], recent_commits: List[Dict[str, Any]]) -> bool:
    ...
    phase_indicators = [
        'major', 'complete', 'finished', 'final', 'refactor', 'reorganize',
        'new feature', 'implementation', 'foundation', 'core', 'advanced'
    ]
...
def analyze_development_patterns(commits: List[Dict[str, Any]], phases: Dict[str, Any]) -> Dict[str, Any]:
...
def generate_timeline(phases: Dict[str, Any], months_back: int) -> Dict[str, Any]:
    ...
    total_duration = sum(
        parse_duration(phase.get('duration', '0 days'))
        for phase in phases.values()
    )
...
def extract_lessons_learned(phases: Dict[str, Any], patterns: Dict[str, Any]) -> Dict[str, str]:
    ...
    if integration_points:
        lessons['integration_strategy'] = f"Incremental integration: {', '.join(integration_points)}"
    else:
...
def create_template_structure(phases: Dict[str, Any], patterns: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "name": f"Repository-Based Template",
...
def create_project_template(project_plan: Dict[str, Any], template_name: str) -> Dict[str, Any]:
    ...
    return {
        "name": template_name,
        "description": f"Template based on {project_plan['project']['name']} development patterns",
        "phases": template_phases,
