...
async def dashboard_home(request: Request):
    ...
    for dept_name, agents in departments.items():
        dept_url = dept_name.lower().replace(' ', '-')
        html_content += f"""
...
async def department_page(request: Request, dept_name: str):
    ...
    for agent in dept_agents:
        ...
        for i, (category, actions) in enumerate(list(action_categories.items())[:3]):
            html_content += f"""
...
async def agent_detail_page(request: Request, agent_id: str):
    ...
    try:
        ...
        if not agent_info:
            return HTMLResponse(content="<h1>Agent not found</h1><p>Agent ID: " + agent_id + "</p>", status_code=404)
        ...
        return HTMLResponse(content=f"<h1>Error loading agent</h1><p>Error: {str(e)}</p>", status_code=500)
    ...
    return {
        ...
        "agents": {
            agent_id: {
                ...
                "department": next(
                    (dept for dept, agents in agent_data["team"].items() 
                     for agent in agents if agent["agent"] == agent_id), 
                    "unknown"
                ),
                "risk_tier": agent_config.get("risk_tier", "medium"),
...
async def execute_agent_action(agent_id: str, action: str, params: dict = None):
...
async def get_department_status(dept_name: str):
...
async def m365_empire_overview(request: Request):
...
async def agent_tasks_page(request: Request, agent_id: str):
    ...
    try:
        ...
        return HTMLResponse(content=f"<h1>Error loading tasks</h1><p>Error: {str(e)}</p>", status_code=500)
