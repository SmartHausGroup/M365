...
def test_e6g_builds_blocked_project_management_contract_pack(
...
def test_e6g_fails_closed_on_declared_coverage_status_mismatch(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setenv("APP_DATA", str(tmp_path))
    ...
    source = Path("registry/department_pack_project_management_v1.yaml")
    payload = yaml.safe_load(source.read_text(encoding="utf-8"))
    payload["personas"]["experiment-tracker"]["coverage_status"] = "registry-backed"
