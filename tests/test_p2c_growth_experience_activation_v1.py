...
def test_p2c_all_personas_registry_backed():
...
def test_p2c_action_counts_match():
    agents, persona_reg, cap_map = _load_registries()
    for pid, (expected, dept) in P2C_PERSONAS.items():
...
def test_p2c_capability_map_parity():
    agents, persona_reg, cap_map = _load_registries()
    for pid, (expected, dept) in P2C_PERSONAS.items():
...
def test_p2c_total_actions():
    agents, _, _ = _load_registries()
    total = sum(
        len(agents[pid]["allowed_actions"]) for pid in P2C_PERSONAS
    )
    assert total == 64


def test_p2c_growth_hacker_has_approval_rules():
    agents, _, _ = _load_registries()
...
def test_p2c_department_packs_updated():
    repo = Path(__file__).resolve().parents[1]
