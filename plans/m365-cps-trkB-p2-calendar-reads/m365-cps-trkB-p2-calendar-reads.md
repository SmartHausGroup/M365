# Parent Phase B2 — Calendar Reads

## Section 1: Plan Header

- **Plan ID:** `plan:m365-cps-trkB-p2-calendar-reads`
- **Parent Plan ID:** `plan:m365-cps-trkB-coverage`
- **Title:** Parent Phase B2 — Calendar Reads
- **Version:** 1.0
- **Status:** draft
...
- **Execution Plan Ref:** docs/platform/EXECUTION_PLAN.md - M365 CPS Track B P2
- **Domain:** framework
...
- **User's stated requirements:**
  - Add calendar.list, calendar.get, events.list, calendar.availability to runtime + alias + YAML.
- **Intent doc ref:** captured in this plan
- **Intent verification:** Tasks T1..T11 together satisfy this parent phase's objective.

## Section 4: Objective

- **Objective:** Add calendar.list, calendar.get, events.list, calendar.availability to runtime + alias + YAML.
...
- Add graph.calendar.list registry entry
- Add graph.calendar.get registry entry
- Add graph.events.list registry entry
- Add graph.calendar.availability registry entry
...
- `Files outside calendar coverage scope`
...
- **Ref:** `plan:m365-cps-trkB-p2-calendar-reads:R4`
...
- **Ref:** `plan:m365-cps-trkB-p2-calendar-reads:R8`
...
- **Ref:** `plan:m365-cps-trkB-p2-calendar-reads:T1`
...
### T2 — Add graph.calendar.list registry entry

- **Ref:** `plan:m365-cps-trkB-p2-calendar-reads:T2`
- **Description:** Add ActionSpec for graph.calendar.list (workload exchange; auth_modes ['auth_code_pkce', 'device_code']; scopes ['Calendars.Read']; risk low; rw read) to READ_ONLY_REGISTRY in src/m365_runtime/graph/registry.py. Endpoint per parent plan B-action spec table.
- **Requirement refs:** R4
- **Depends on:** T1
- **MATHS phase:** N/A
- **Deliverables:**
  - src/m365_runtime/graph/registry.py with graph.calendar.list ActionSpec
- **Exit criteria:**
  - get_action('graph.calendar.list') returns the spec; admit() works
- **Validation command:** `cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import get_action; spec = get_action('graph.calendar.list'); assert spec.workload == 'exchange'; print('OK')"`
- **Files to modify:**
  - M365: src/m365_runtime/graph/registry.py

### T3 — Add graph.calendar.get registry entry

- **Ref:** `plan:m365-cps-trkB-p2-calendar-reads:T3`
- **Description:** Add ActionSpec for graph.calendar.get (workload exchange; auth_modes ['auth_code_pkce', 'device_code']; scopes ['Calendars.Read']; risk low; rw read) to READ_ONLY_REGISTRY in src/m365_runtime/graph/registry.py. Endpoint per parent plan B-action spec table.
- **Requirement refs:** R4
- **Depends on:** T2
- **MATHS phase:** N/A
- **Deliverables:**
  - src/m365_runtime/graph/registry.py with graph.calendar.get ActionSpec
- **Exit criteria:**
  - get_action('graph.calendar.get') returns the spec; admit() works
- **Validation command:** `cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import get_action; spec = get_action('graph.calendar.get'); assert spec.workload == 'exchange'; print('OK')"`
- **Files to modify:**
  - M365: src/m365_runtime/graph/registry.py

### T4 — Add graph.events.list registry entry

- **Ref:** `plan:m365-cps-trkB-p2-calendar-reads:T4`
- **Description:** Add ActionSpec for graph.events.list (workload exchange; auth_modes ['auth_code_pkce', 'app_only_secret', 'app_only_certificate']; scopes ['Calendars.Read']; risk low; rw read) to READ_ONLY_REGISTRY in src/m365_runtime/graph/registry.py. Endpoint per parent plan B-action spec table.
- **Requirement refs:** R4
- **Depends on:** T3
- **MATHS phase:** N/A
- **Deliverables:**
  - src/m365_runtime/graph/registry.py with graph.events.list ActionSpec
- **Exit criteria:**
  - get_action('graph.events.list') returns the spec; admit() works
- **Validation command:** `cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import get_action; spec = get_action('graph.events.list'); assert spec.workload == 'exchange'; print('OK')"`
- **Files to modify:**
  - M365: src/m365_runtime/graph/registry.py

### T5 — Add graph.calendar.availability registry entry

- **Ref:** `plan:m365-cps-trkB-p2-calendar-reads:T5`
- **Description:** Add ActionSpec for graph.calendar.availability (workload exchange; auth_modes ['auth_code_pkce', 'device_code']; scopes ['Calendars.Read']; risk low; rw read) to READ_ONLY_REGISTRY in src/m365_runtime/graph/registry.py. Endpoint per parent plan B-action spec table.
- **Requirement refs:** R4
- **Depends on:** T4
- **MATHS phase:** N/A
- **Deliverables:**
  - src/m365_runtime/graph/registry.py with graph.calendar.availability ActionSpec
- **Exit criteria:**
  - get_action('graph.calendar.availability') returns the spec; admit() works
- **Validation command:** `cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import get_action; spec = get_action('graph.calendar.availability'); assert spec.workload == 'exchange'; print('OK')"`
- **Files to modify:**
  - M365: src/m365_runtime/graph/registry.py

### T6 — Update legacy alias table

- **Ref:** `plan:m365-cps-trkB-p2-calendar-reads:T6`
...
### T7 — Update YAML registry mirror

- **Ref:** `plan:m365-cps-trkB-p2-calendar-reads:T7`
...
- **Ref:** `plan:m365-cps-trkB-p2-calendar-reads:T8`
...
- **Exit criteria:**
  - pytest exits 0
- **Validation command:** `cd $M365_ROOT && .venv/bin/python -m pytest tests/test_m365_pack_client.py -k 'p2' -x`
- **Files to modify:**
  - M365: tests/test_m365_pack_client.py

### T9 — UCP routing unit tests

- **Ref:** `plan:m365-cps-trkB-p2-calendar-reads:T9`
...
- **Exit criteria:**
  - pytest exits 0
- **Validation command:** `cd $UCP_ROOT && .venv/bin/python -m pytest tests/test_m365_integration.py -k 'p2' -x`
- **Files to modify:**
  - UCP: tests/test_m365_integration.py

### T10 — Pre-commit + make test

- **Ref:** `plan:m365-cps-trkB-p2-calendar-reads:T10`
...
- **Ref:** `plan:m365-cps-trkB-p2-calendar-reads:T11`
- **Description:** Commit format 'plan:m365-cps-trkB-p2: T1-T10 <parent name>'.
- **Requirement refs:** R8
- **Depends on:** T10
- **MATHS phase:** N/A
- **Deliverables:**
...
- **Validation command:** `cd $UCP_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p2' && cd $M365_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p2'`

## Section 9: Gate Checks

### CHECK:C0 — Parent phase B2 integration

- **Description:** All child tasks T1..T11 green; integration test pass
- **Task ref:** T1..T11
- **Validation command:** `cd $UCP_ROOT && .venv/bin/python tests/integration/m365_calendar_e2e.py`
...
| src/m365_runtime/graph/registry.py with graph.calendar.list ActionSpec | varies | T2 | per-task | see Section 8 / T2 |
| src/m365_runtime/graph/registry.py with graph.calendar.get ActionSpec | varies | T3 | per-task | see Section 8 / T3 |
| src/m365_runtime/graph/registry.py with graph.events.list ActionSpec | varies | T4 | per-task | see Section 8 / T4 |
| src/m365_runtime/graph/registry.py with graph.calendar.availability ActionSpec | varies | T5 | per-task | see Section 8 / T5 |
| src/ucp_m365_pack/client.py with new alias entries | varies | T6 | per-task | see Section 8 / T6 |
| registry/action_registry.yaml with new entries | varies | T7 | per-task | see Section 8 / T7 |
| Updated test file | varies | T8 | per-task | see Section 8 / T8 |
| Updated UCP test file | varies | T9 | per-task | see Section 8 / T9 |
| pre-commit + make test green in both repos | varies | T10 | per-task | see Section 8 / T10 |
| Commit in both repos | varies | T11 | per-task | see Section 8 / T11 |
...
- **Per-task prompts:**
  - `docs/prompts/codex-m365-cps-trkB-p2-calendar-reads-T1.md` (detailed)
  - `docs/prompts/codex-m365-cps-trkB-p2-calendar-reads-T1-prompt.txt` (kickoff)
  - `docs/prompts/codex-m365-cps-trkB-p2-calendar-reads-T2.md` (detailed)
  - `docs/prompts/codex-m365-cps-trkB-p2-calendar-reads-T2-prompt.txt` (kickoff)
  - `docs/prompts/codex-m365-cps-trkB-p2-calendar-reads-T3.md` (detailed)
  - `docs/prompts/codex-m365-cps-trkB-p2-calendar-reads-T3-prompt.txt` (kickoff)
  - `docs/prompts/codex-m365-cps-trkB-p2-calendar-reads-T4.md` (detailed)
  - `docs/prompts/codex-m365-cps-trkB-p2-calendar-reads-T4-prompt.txt` (kickoff)
  - `docs/prompts/codex-m365-cps-trkB-p2-calendar-reads-T5.md` (detailed)
  - `docs/prompts/codex-m365-cps-trkB-p2-calendar-reads-T5-prompt.txt` (kickoff)
  - `docs/prompts/codex-m365-cps-trkB-p2-calendar-reads-T6.md` (detailed)
  - `docs/prompts/codex-m365-cps-trkB-p2-calendar-reads-T6-prompt.txt` (kickoff)
  - `docs/prompts/codex-m365-cps-trkB-p2-calendar-reads-T7.md` (detailed)
  - `docs/prompts/codex-m365-cps-trkB-p2-calendar-reads-T7-prompt.txt` (kickoff)
  - `docs/prompts/codex-m365-cps-trkB-p2-calendar-reads-T8.md` (detailed)
  - `docs/prompts/codex-m365-cps-trkB-p2-calendar-reads-T8-prompt.txt` (kickoff)
  - `docs/prompts/codex-m365-cps-trkB-p2-calendar-reads-T9.md` (detailed)
  - `docs/prompts/codex-m365-cps-trkB-p2-calendar-reads-T9-prompt.txt` (kickoff)
  - `docs/prompts/codex-m365-cps-trkB-p2-calendar-reads-T10.md` (detailed)
  - `docs/prompts/codex-m365-cps-trkB-p2-calendar-reads-T10-prompt.txt` (kickoff)
  - `docs/prompts/codex-m365-cps-trkB-p2-calendar-reads-T11.md` (detailed)
  - `docs/prompts/codex-m365-cps-trkB-p2-calendar-reads-T11-prompt.txt` (kickoff)
...
- [ ] T2 — Add graph.calendar.list registry entry — Evidence:
- [ ] T3 — Add graph.calendar.get registry entry — Evidence:
- [ ] T4 — Add graph.events.list registry entry — Evidence:
- [ ] T5 — Add graph.calendar.availability registry entry — Evidence:
- [ ] T6 — Update legacy alias table — Evidence:
- [ ] T7 — Update YAML registry mirror — Evidence:
- [ ] T8 — M365 unit tests — Evidence:
- [ ] T9 — UCP routing unit tests — Evidence:
- [ ] T10 — Pre-commit + make test — Evidence:
- [ ] T11 — Commit (no push) — Evidence:

### Gate checklist

- [ ] CHECK:C0 — Parent phase B2 integration — Result:
