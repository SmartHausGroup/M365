...
def main() -> int:
    ...
    if summary["active_persona_count"] != 5:
        raise SystemExit("testing_department_pack_active_count_mismatch")
    if summary["registry_backed_persona_count"] != 5:
        raise SystemExit("testing_department_pack_registry_backed_count_mismatch")
    if summary["supported_action_count"] != 38:
        raise SystemExit("testing_department_pack_supported_action_count_mismatch")
    if summary["pack_state"] != "ready":
        raise SystemExit("testing_department_pack_expected_ready")
    ...
    output_path = (
        repo_root / "configs" / "generated" / "testing_department_pack_v1_verification.json"
    )
