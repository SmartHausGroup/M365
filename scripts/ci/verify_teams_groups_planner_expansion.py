...
REGISTRY_PATH = REPO_ROOT / "registry" / "teams_groups_planner_expansion_v2.yaml"
CAPABILITY_REGISTRY_PATH = REPO_ROOT / "registry" / "capability_registry.yaml"
ARTIFACT_PATH = (
    REPO_ROOT / "configs" / "generated" / "teams_groups_planner_expansion_verification.json"
)
...
def main() -> int:
    ...
    for action, definition in supported_actions.items():
        ...
        capability_alias = definition.get("capability_registry_alias")
        capability_entry = capability_actions.get(str(capability_alias))
        if not capability_entry or capability_entry.get("status") != "implemented":
            capability_mismatches.append(action)

        if executor_route_for_action(None, action) != "collaboration":
        ...
        if auth.auth_class != expected_auth_class or auth.executor_domain != "collaboration":
    ...
    print(
        "verify_teams_groups_planner_expansion:",
