...
def main() -> int:
    ...
    for action, definition in supported_actions.items():
        ...
        expected_domain = str(definition.get("executor_domain") or "")
        if executor_route_for_action(None, action) != expected_domain:
            route_mismatches.append(action)

        auth = resolve_action_auth("admin-governance", action, {})
        if auth.auth_class != "app_only" or auth.executor_domain != expected_domain:
    ...
    print(
        "verify_admin_governance_surface_expansion:",
