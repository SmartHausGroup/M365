...
REGISTRY_PATH = REPO_ROOT / "registry" / "security_defender_expansion_v2.yaml"
CAPABILITY_REGISTRY_PATH = REPO_ROOT / "registry" / "capability_registry.yaml"
ARTIFACT_PATH = REPO_ROOT / "configs" / "generated" / "security_defender_expansion_verification.json"
...
def main() -> int:
    ...
    for action, definition in supported_actions.items():
        ...
        if executor_route_for_action(None, action) != "security":
            route_mismatches.append(action)

        auth = resolve_action_auth("security-operations", action, {})
        if auth.auth_class != "app_only" or auth.executor_domain != "security":
    ...
    print(
        "verify_security_defender_expansion:",
