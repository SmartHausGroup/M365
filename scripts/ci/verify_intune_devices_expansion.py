...
REGISTRY_PATH = REPO_ROOT / "registry" / "intune_devices_expansion_v2.yaml"
CAPABILITY_REGISTRY_PATH = REPO_ROOT / "registry" / "capability_registry.yaml"
ARTIFACT_PATH = REPO_ROOT / "configs" / "generated" / "intune_devices_expansion_verification.json"
...
def main() -> int:
    ...
    for action, definition in supported_actions.items():
        ...
        if executor_route_for_action(None, action) != "devices":
            route_mismatches.append(action)

        auth = resolve_action_auth("m365-administrator", action, {})
        if auth.auth_class != "app_only" or auth.executor_domain != "devices":
    ...
    print(
        "verify_intune_devices_expansion:",
