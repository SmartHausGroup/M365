# Codex Execution Prompt — plan:m365-cps-trkC-p2-agents-yaml-schema:T3

## Governance Lock (Read FIRST)

- `AGENTS.md` / `CLAUDE.md` (M365 repo and UCP repo)
- `docs/NORTH_STAR.md` (UCP) / `Operations/NORTHSTAR.md` (M365)
- `plans/m365-cps-trkC-p2-agents-yaml-schema/m365-cps-trkC-p2-agents-yaml-schema.md` (parent phase plan)
...
- **Task name:** Update validate_agent_action
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T2
...
validate_agent_action returns coverage_status alongside the boolean.

Plan reference: `plan:m365-cps-trkC-p2-agents-yaml-schema:T3`. Branch: `feat/m365-cps-trkC`.
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python -c "from ucp_m365_pack.client import validate_agent_action; result = validate_agent_action('m365-administrator', 'sites.list'); print(result)"
...
git add <files>
git commit -m "plan:m365-cps-trkC-p2-agents-yaml-schema:T3 — Update validate_agent_action"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python -c "from ucp_m365_pack.client import validate_agent_action; result = validate_agent_action('m365-administrator', 'sites.list'); print(result)"`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkC-p2-agents-yaml-schema:T3`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python -c "from ucp_m365_pack.client import validate_agent_action; result = validate_agent_action('m365-administrator', 'sites.list'); print(result)"` exits 0
...
- Reference `plan:m365-cps-trkC-p2-agents-yaml-schema:T3` in commit message
