# Codex Execution Prompt — plan:m365-cps-trkC-p3-mcp-tool-docstrings:T2
...
- **Task:** `T2` — Update docstrings with coverage_status markers
- **Branch:** `feat/m365-cps-trkC`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkC-p3-mcp-tool-docstrings-T2
...
- **Task name:** Update docstrings with coverage_status markers
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T1
...
Each documented action in MCP tool docstrings gets a marker: (implemented), (aliased->graph.X), (planned).

Plan reference: `plan:m365-cps-trkC-p3-mcp-tool-docstrings:T2`. Branch: `feat/m365-cps-trkC`.
...
# 3. Validate
cd $UCP_ROOT && .venv/bin/python scripts/check_docstring_markers.py
...
git add <files>
git commit -m "plan:m365-cps-trkC-p3-mcp-tool-docstrings:T2 — Update docstrings with coverage_status markers"
```
...
- Validation command exits 0: `cd $UCP_ROOT && .venv/bin/python scripts/check_docstring_markers.py`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkC-p3-mcp-tool-docstrings:T2`
...
- **Validation result:** `cd $UCP_ROOT && .venv/bin/python scripts/check_docstring_markers.py` exits 0
...
- Reference `plan:m365-cps-trkC-p3-mcp-tool-docstrings:T2` in commit message
