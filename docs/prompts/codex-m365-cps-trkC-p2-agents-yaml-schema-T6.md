# Codex Execution Prompt — plan:m365-cps-trkC-p2-agents-yaml-schema:T6
...
- **Task:** `T6` — Update UCP m365_tools to surface coverage_status
- **Branch:** `feat/m365-cps-trkC`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkC-p2-agents-yaml-schema-T6
...
- **Task name:** Update UCP m365_tools to surface coverage_status
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T5
...
MCP tool responses include coverage_status when relevant; not_yet_implemented status surfaces clearly.

Plan reference: `plan:m365-cps-trkC-p2-agents-yaml-schema:T6`. Branch: `feat/m365-cps-trkC`.
...
# 3. Validate
cd $UCP_ROOT && .venv/bin/python -m pytest tests/test_m365_integration.py -k coverage_status -x
...
git add <files>
git commit -m "plan:m365-cps-trkC-p2-agents-yaml-schema:T6 — Update UCP m365_tools to surface coverage_status"
```
...
- Validation command exits 0: `cd $UCP_ROOT && .venv/bin/python -m pytest tests/test_m365_integration.py -k coverage_status -x`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkC-p2-agents-yaml-schema:T6`
...
- **Validation result:** `cd $UCP_ROOT && .venv/bin/python -m pytest tests/test_m365_integration.py -k coverage_status -x` exits 0
...
- Reference `plan:m365-cps-trkC-p2-agents-yaml-schema:T6` in commit message
