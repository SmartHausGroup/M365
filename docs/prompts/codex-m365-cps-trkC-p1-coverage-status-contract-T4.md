# Codex Execution Prompt — plan:m365-cps-trkC-p1-coverage-status-contract:T4
...
- **Task:** `T4` — Pre-commit + commit
- **Branch:** `feat/m365-cps-trkC`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkC-p1-coverage-status-contract-T4
...
Plan reference: `plan:m365-cps-trkC-p1-coverage-status-contract:T4`. Branch: `feat/m365-cps-trkC`.
...
# 3. Validate
cd $M365_ROOT && pre-commit run --all-files && make test && git log --oneline -1 | grep 'plan:m365-cps-trkC-p1'
...
git add <files>
git commit -m "plan:m365-cps-trkC-p1-coverage-status-contract:T4 — Pre-commit + commit"
```
...
- Validation command exits 0: `cd $M365_ROOT && pre-commit run --all-files && make test && git log --oneline -1 | grep 'plan:m365-cps-trkC-p1'`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkC-p1-coverage-status-contract:T4`
...
- **Validation result:** `cd $M365_ROOT && pre-commit run --all-files && make test && git log --oneline -1 | grep 'plan:m365-cps-trkC-p1'` exits 0
...
- Reference `plan:m365-cps-trkC-p1-coverage-status-contract:T4` in commit message
