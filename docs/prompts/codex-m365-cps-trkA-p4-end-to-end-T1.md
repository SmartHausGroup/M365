# Codex Execution Prompt — plan:m365-cps-trkA-p4-end-to-end:T1
...
- **Task:** `T1` — Live UCP+M365 spin-up integration test
- **Branch:** `feat/m365-cps-trkA`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkA-p4-end-to-end-T1
...
- **Task name:** Live UCP+M365 spin-up integration test
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** none
...
Spin up UCP MCP server and M365 runtime locally. Authenticate via device-code. Run end-to-end: m365_inventory call, m365_auth_check call, unknown action call.

Plan reference: `plan:m365-cps-trkA-p4-end-to-end:T1`. Branch: `feat/m365-cps-trkA`.
...
1. Per parent plan requirements: R1, R2, R3
...
# 3. Validate
cd $UCP_ROOT && .venv/bin/python tests/integration/m365_cps_trkA_e2e.py
...
git add <files>
git commit -m "plan:m365-cps-trkA-p4-end-to-end:T1 — Live UCP+M365 spin-up integration test"
```
...
- Validation command exits 0: `cd $UCP_ROOT && .venv/bin/python tests/integration/m365_cps_trkA_e2e.py`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkA-p4-end-to-end:T1`
...
- **Validation result:** `cd $UCP_ROOT && .venv/bin/python tests/integration/m365_cps_trkA_e2e.py` exits 0
...
- Reference `plan:m365-cps-trkA-p4-end-to-end:T1` in commit message
