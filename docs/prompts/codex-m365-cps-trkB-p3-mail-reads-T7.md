# Codex Execution Prompt — plan:m365-cps-trkB-p3-mail-reads:T7
...
- **Task:** `T7` — M365 unit tests
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p3-mail-reads-T7
...
Plan reference: `plan:m365-cps-trkB-p3-mail-reads:T7`. Branch: `feat/m365-cps-trkB`.
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python -m pytest tests/test_m365_pack_client.py -k 'p3' -x
...
git add <files>
git commit -m "plan:m365-cps-trkB-p3-mail-reads:T7 — M365 unit tests"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python -m pytest tests/test_m365_pack_client.py -k 'p3' -x`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p3-mail-reads:T7`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python -m pytest tests/test_m365_pack_client.py -k 'p3' -x` exits 0
...
- Reference `plan:m365-cps-trkB-p3-mail-reads:T7` in commit message
