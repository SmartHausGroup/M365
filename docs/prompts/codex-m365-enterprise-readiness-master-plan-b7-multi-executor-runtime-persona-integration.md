...
- Domain: `code`
- Dependencies: `B6E`
- Goal: implement the rebased production target with bounded executor domains, persona-aware routing, and approval-path reachability.
...
- `M365-READY-B7-C4` minimize executor permissions and clean Azure posture.
- `M365-READY-B7-C5` re-prove approval reachability.
- `M365-READY-B7-C6` refresh certification readiness.
- `M365-READY-B7-C7` emit final gate and next-act state.

## Validation

1. `rg -n "executor|persona|routing|approval backend|SharePoint executor|B7" src docs plans`
2. `git diff --check`
