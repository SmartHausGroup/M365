# Codex Execution Prompt — plan:m365-cps-trkA-p3-preflight-intersection:T3
...
- **Task:** `T3` — Surface in /v1/auth/check response
- **Branch:** `feat/m365-cps-trkA`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkA-p3-preflight-intersection-T3
...
- **Task name:** Surface in /v1/auth/check response
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T2
...
Update auth_check handler in m365_runtime to call compute_intersection and include the result under 'preflight' key in response.

Plan reference: `plan:m365-cps-trkA-p3-preflight-intersection:T3`. Branch: `feat/m365-cps-trkA`.
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python -m pytest tests/test_m365_auth_check.py -k preflight -x
...
git add <files>
git commit -m "plan:m365-cps-trkA-p3-preflight-intersection:T3 — Surface in /v1/auth/check response"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python -m pytest tests/test_m365_auth_check.py -k preflight -x`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkA-p3-preflight-intersection:T3`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python -m pytest tests/test_m365_auth_check.py -k preflight -x` exits 0
...
- Reference `plan:m365-cps-trkA-p3-preflight-intersection:T3` in commit message
