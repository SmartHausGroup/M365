# Codex Execution Prompt — plan:m365-cps-trkB-p6-auth-mode-tiers:T2
...
- **Task:** `T2` — Update m365_auth_start to accept tier
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p6-auth-mode-tiers-T2
...
- **Task name:** Update m365_auth_start to accept tier
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T1
...
Add tier parameter to m365_auth_start endpoint and OAuth flow. Default tier=read-only.

Plan reference: `plan:m365-cps-trkB-p6-auth-mode-tiers:T2`. Branch: `feat/m365-cps-trkB`.
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python -m pytest tests/test_m365_auth_tiers.py -x
...
git add <files>
git commit -m "plan:m365-cps-trkB-p6-auth-mode-tiers:T2 — Update m365_auth_start to accept tier"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python -m pytest tests/test_m365_auth_tiers.py -x`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p6-auth-mode-tiers:T2`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python -m pytest tests/test_m365_auth_tiers.py -x` exits 0
...
- Reference `plan:m365-cps-trkB-p6-auth-mode-tiers:T2` in commit message
