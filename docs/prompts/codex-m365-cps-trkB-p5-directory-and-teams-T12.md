# Codex Execution Prompt — plan:m365-cps-trkB-p5-directory-and-teams:T12
...
- **Task:** `T12` — Commit (no push)
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p5-directory-and-teams-T12
...
Commit format 'plan:m365-cps-trkB-p5: T1-T11 <parent name>'.

Plan reference: `plan:m365-cps-trkB-p5-directory-and-teams:T12`. Branch: `feat/m365-cps-trkB`.
...
- Cross-task changes (this prompt is scoped to T12 only)
...
# 3. Validate
cd $UCP_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p5' && cd $M365_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p5'
...
git add <files>
git commit -m "plan:m365-cps-trkB-p5-directory-and-teams:T12 — Commit (no push)"
```
...
- Validation command exits 0: `cd $UCP_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p5' && cd $M365_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p5'`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p5-directory-and-teams:T12`
...
- **Validation result:** `cd $UCP_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p5' && cd $M365_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p5'` exits 0
...
- Reference `plan:m365-cps-trkB-p5-directory-and-teams:T12` in commit message
