# Codex Execution Prompt — plan:m365-cps-trkC-p4-operator-capability-map:T4
...
- **Task:** `T4` — Make regeneration a CI artifact
- **Branch:** `feat/m365-cps-trkC`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkC-p4-operator-capability-map-T4
...
- **Task name:** Make regeneration a CI artifact
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T3
...
Pre-commit hook regenerates the doc on registry/agents.yaml changes; fails if doc out-of-date.

Plan reference: `plan:m365-cps-trkC-p4-operator-capability-map:T4`. Branch: `feat/m365-cps-trkC`.
...
# 3. Validate
cd $M365_ROOT && pre-commit run generate-capability-map
...
git add <files>
git commit -m "plan:m365-cps-trkC-p4-operator-capability-map:T4 — Make regeneration a CI artifact"
```
...
- Validation command exits 0: `cd $M365_ROOT && pre-commit run generate-capability-map`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkC-p4-operator-capability-map:T4`
...
- **Validation result:** `cd $M365_ROOT && pre-commit run generate-capability-map` exits 0
...
- Reference `plan:m365-cps-trkC-p4-operator-capability-map:T4` in commit message
