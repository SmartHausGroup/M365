# MATHS Prompt: E6B HR Department Pack
...
- `PLAN_REF_ACK: plan:m365-ai-workforce-expansion-master-plan:E6B`
- `NORTH_STAR_ACK: Operations/NORTHSTAR.md`
- `RULES_ACK: .cursor/rules/**/*.mdc`

## Execution Rules

- Run checks `M365-WORKFORCE-E6B-C0` -> `M365-WORKFORCE-E6B-C10` in strict order.
- Stop on first `FAIL` or `BLOCKED`.
- Output each check exactly:
  - `CHECK:<id> STATUS:<PASS|FAIL|BLOCKED> EVIDENCE:<path/command> NOTE:<one line>`
- Final outputs exactly:
  - `GATE:M365-WORKFORCE-E6B STATUS:<GO|NO-GO>`
  - `FINAL_DECISION:<GO|NO-GO>`

## Prompt Run Metadata

- Prompt version: `1.0`
- Task ID: `M365-WORKFORCE-E6B`
- Run ID: `hr-department-pack`
...
- Plan refs in scope:
  ...
  - `plan:m365-ai-workforce-expansion-master-plan:E6B`
...
- Success criteria:
  - `E6B` produces its declared artifacts or runtime changes without scope drift.
  - `E6B` updates plan and tracker state so the next dependency edge is explicit.
  - `E6B` leaves deterministic evidence paths and validation commands for replay.
- Out of scope:
...
- Artifact/schema evidence:
  - `Artifacts created or updated for E6B exist with required metadata, status, and dependency fields.`
- Runtime/test evidence:
  - `Act-scoped validation commands for E6B are recorded with expected pass/fail semantics.`
- Governance evidence:
...
- Determinism evidence:
  - `A fixed-state replay of the act leaves the same plan status, prompt inventory, and validation signatures for E6B.`

## T - Tie

- Dependency ties:
  - `E6 -> E6B may not claim GO unless predecessors are satisfied.`
  - `E6B -> downstream acts inherit its outputs and must stay blocked if E6B is NO-GO.`
- Known failure modes:
...
- GO criteria:
  - `All M365-WORKFORCE-E6B-C0 through M365-WORKFORCE-E6B-C10 checks pass with required evidence.`
- NO-GO criteria:
  - `Any missing required artifact, failed hard gate, unresolved blocker, or replay mismatch.`

## H - Harness (ordered checks)

`M365-WORKFORCE-E6B-C0` Preflight

- Verify governance docs, plan refs, and prerequisite phase status.

`M365-WORKFORCE-E6B-C1` Baseline inventory

- Capture the current baseline for E6B, including artifact presence, phase status, and dependency state.

`M365-WORKFORCE-E6B-C2` Intent and boundary lock

- Confirm the exact objective, success criteria, and out-of-scope boundary for E6B.

`M365-WORKFORCE-E6B-C3` Implementation or specification step 1

- Apply the primary deterministic change required for E6B.

`M365-WORKFORCE-E6B-C4` Implementation or specification step 2

- Apply the secondary deterministic change required for E6B.

`M365-WORKFORCE-E6B-C5` Schema and contract validation

- Hard-fail on missing required keys, statuses, links, or metadata.

`M365-WORKFORCE-E6B-C6` Targeted validations

- Execute the act-scoped validation commands for E6B.

`M365-WORKFORCE-E6B-C7` Strict artifact validation

- Verify required paths exist and reflect the intended state only.

`M365-WORKFORCE-E6B-C8` Deterministic replay

- Repeat the fixed-state validation and require consistent outputs.

`M365-WORKFORCE-E6B-C9` Hard gates (strict order)

1. `Artifact presence and schema checks`
2. `Targeted act validation command(s)`
3. `git diff --check or approved equivalent`

`M365-WORKFORCE-E6B-C10` Governance synchronization and final decision
...
- Replay checks:
  - `The plan status, prompt inventory, and act-level validation outputs for E6B must remain stable across repeated runs on fixed repo state.`

## Output Contract

- Deliverables:
  - `E6B` artifacts and synchronized tracker state
  - `E6B` validation evidence
- Validation results:
  - `M365-WORKFORCE-E6B-C0..C10 statuses`
- Evidence links:
