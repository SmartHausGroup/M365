# Codex Execution Prompt — plan:m365-cps-trkC-p4-operator-capability-map:T1
...
- **Task:** `T1` — Generate capability map document
- **Branch:** `feat/m365-cps-trkC`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkC-p4-operator-capability-map-T1
...
- **Task name:** Generate capability map document
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** none
...
scripts/generate_capability_map.py reads registry + alias table + agents.yaml and emits docs/m365_capability_map.md with: per-agent allowed actions, per-action coverage_status, per-action runtime endpoint and scopes.

Plan reference: `plan:m365-cps-trkC-p4-operator-capability-map:T1`. Branch: `feat/m365-cps-trkC`.
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python scripts/generate_capability_map.py > docs/m365_capability_map.md
...
git add <files>
git commit -m "plan:m365-cps-trkC-p4-operator-capability-map:T1 — Generate capability map document"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python scripts/generate_capability_map.py > docs/m365_capability_map.md`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkC-p4-operator-capability-map:T1`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python scripts/generate_capability_map.py > docs/m365_capability_map.md` exits 0
...
- Reference `plan:m365-cps-trkC-p4-operator-capability-map:T1` in commit message
