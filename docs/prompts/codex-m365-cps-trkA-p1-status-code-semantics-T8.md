# Codex Execution Prompt — plan:m365-cps-trkA-p1-status-code-semantics:T8
...
- **Task:** `T8` — Commit child phases (no push)
- **Branch:** `feat/m365-cps-trkA`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkA-p1-status-code-semantics-T8
...
- **Task name:** Commit child phases (no push)
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T7
...
Create commits in both repos with message format 'plan:m365-cps-trkA-p1: T1-T7 status code semantics'. No push.

Plan reference: `plan:m365-cps-trkA-p1-status-code-semantics:T8`. Branch: `feat/m365-cps-trkA`.
...
# 3. Validate
cd $UCP_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkA-p1' && cd $M365_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkA-p1'
...
git add <files>
git commit -m "plan:m365-cps-trkA-p1-status-code-semantics:T8 — Commit child phases (no push)"
```
...
- Validation command exits 0: `cd $UCP_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkA-p1' && cd $M365_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkA-p1'`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkA-p1-status-code-semantics:T8`
...
- **Validation result:** `cd $UCP_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkA-p1' && cd $M365_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkA-p1'` exits 0
...
- Reference `plan:m365-cps-trkA-p1-status-code-semantics:T8` in commit message
