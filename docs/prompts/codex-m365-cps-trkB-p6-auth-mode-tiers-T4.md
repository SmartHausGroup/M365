# Codex Execution Prompt — plan:m365-cps-trkB-p6-auth-mode-tiers:T4

## Governance Lock (Read FIRST)

- `AGENTS.md` / `CLAUDE.md` (M365 repo and UCP repo)
- `docs/NORTH_STAR.md` (UCP) / `Operations/NORTHSTAR.md` (M365)
- `plans/m365-cps-trkB-p6-auth-mode-tiers/m365-cps-trkB-p6-auth-mode-tiers.md` (parent phase plan)
...
- **Plan:** `plan:m365-cps-trkB-p6-auth-mode-tiers`
- **Parent track:** `plan:m365-cps-trkB-coverage`
- **Master plan:** `plan:m365-capability-pack-surface-remediation`
- **Task:** `T4` — Update registry actions to declare min_tier
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p6-auth-mode-tiers-T4
...
- **Task name:** Update registry actions to declare min_tier
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T3
...
Each ActionSpec gains min_tier field (read-only by default; admin reads use admin).

Plan reference: `plan:m365-cps-trkB-p6-auth-mode-tiers:T4`. Branch: `feat/m365-cps-trkB`.
...
1. Per parent plan requirements: R5
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import get_action; assert get_action('graph.users.list').min_tier == 'read-only'"
...
git add <files>
git commit -m "plan:m365-cps-trkB-p6-auth-mode-tiers:T4 — Update registry actions to declare min_tier"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import get_action; assert get_action('graph.users.list').min_tier == 'read-only'"`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p6-auth-mode-tiers:T4`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import get_action; assert get_action('graph.users.list').min_tier == 'read-only'"` exits 0
...
- Reference `plan:m365-cps-trkB-p6-auth-mode-tiers:T4` in commit message
