# Codex Execution Prompt — plan:m365-cps-trkA-p4-end-to-end:T3
...
- **Task:** `T3` — Verify pre-flight diagnostic against live tenant
- **Branch:** `feat/m365-cps-trkA`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkA-p4-end-to-end-T3
...
- **Task name:** Verify pre-flight diagnostic against live tenant
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T2
...
Call m365_auth_check after sign-in. Validate preflight.invokable_actions matches device-code-compatible actions and blocked_by_auth_mode shows the rest.

Plan reference: `plan:m365-cps-trkA-p4-end-to-end:T3`. Branch: `feat/m365-cps-trkA`.
...
# 3. Validate
cd $UCP_ROOT && .venv/bin/python tests/integration/m365_preflight_live.py
...
git add <files>
git commit -m "plan:m365-cps-trkA-p4-end-to-end:T3 — Verify pre-flight diagnostic against live tenant"
```
...
- Validation command exits 0: `cd $UCP_ROOT && .venv/bin/python tests/integration/m365_preflight_live.py`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkA-p4-end-to-end:T3`
...
- **Validation result:** `cd $UCP_ROOT && .venv/bin/python tests/integration/m365_preflight_live.py` exits 0
...
- Reference `plan:m365-cps-trkA-p4-end-to-end:T3` in commit message
