# Codex Execution Prompt — plan:m365-cps-trkA-p1-status-code-semantics:T6
...
- **Task:** `T6` — Add UCP unit tests for unknown_action routing
- **Branch:** `feat/m365-cps-trkA`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkA-p1-status-code-semantics-T6
...
- **Task name:** Add UCP unit tests for unknown_action routing
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T5
...
Add test_m365_unknown_action_routing in tests/test_m365_integration.py.

Plan reference: `plan:m365-cps-trkA-p1-status-code-semantics:T6`. Branch: `feat/m365-cps-trkA`.
...
# 3. Validate
cd $UCP_ROOT && .venv/bin/python -m pytest tests/test_m365_integration.py -k m365_unknown -x
...
git add <files>
git commit -m "plan:m365-cps-trkA-p1-status-code-semantics:T6 — Add UCP unit tests for unknown_action routing"
```
...
- Validation command exits 0: `cd $UCP_ROOT && .venv/bin/python -m pytest tests/test_m365_integration.py -k m365_unknown -x`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkA-p1-status-code-semantics:T6`
...
- **Validation result:** `cd $UCP_ROOT && .venv/bin/python -m pytest tests/test_m365_integration.py -k m365_unknown -x` exits 0
...
- Reference `plan:m365-cps-trkA-p1-status-code-semantics:T6` in commit message
