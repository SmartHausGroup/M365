# Codex Execution Prompt — plan:m365-cps-trkC-p1-coverage-status-contract:T3

## Governance Lock (Read FIRST)

- `AGENTS.md` / `CLAUDE.md` (M365 repo and UCP repo)
- `docs/NORTH_STAR.md` (UCP) / `Operations/NORTHSTAR.md` (M365)
- `plans/m365-cps-trkC-p1-coverage-status-contract/m365-cps-trkC-p1-coverage-status-contract.md` (parent phase plan)
...
- **Plan:** `plan:m365-cps-trkC-p1-coverage-status-contract`
- **Parent track:** `plan:m365-cps-trkC-truth-in-advertising`
- **Master plan:** `plan:m365-capability-pack-surface-remediation`
- **Task:** `T3` — Add not_yet_implemented to status enum
- **Branch:** `feat/m365-cps-trkC`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkC-p1-coverage-status-contract-T3
...
- **Task name:** Add not_yet_implemented to status enum
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T2
...
Add not_yet_implemented to runtime status_class enum._denial_to_status returns it for status=planned actions.

Plan reference: `plan:m365-cps-trkC-p1-coverage-status-contract:T3`. Branch: `feat/m365-cps-trkC`.
...
1. Per parent plan requirements: R6
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import _denial_to_status; assert _denial_to_status('planned') == 'not_yet_implemented'"
...
git add <files>
git commit -m "plan:m365-cps-trkC-p1-coverage-status-contract:T3 — Add not_yet_implemented to status enum"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import _denial_to_status; assert _denial_to_status('planned') == 'not_yet_implemented'"`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkC-p1-coverage-status-contract:T3`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import _denial_to_status; assert _denial_to_status('planned') == 'not_yet_implemented'"` exits 0
...
- Reference `plan:m365-cps-trkC-p1-coverage-status-contract:T3` in commit message
