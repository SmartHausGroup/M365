# Codex Execution Prompt — plan:m365-cps-trkA-p1-status-code-semantics:T7
...
- **Task:** `T7` — Pre-commit + make test in both repos
- **Branch:** `feat/m365-cps-trkA`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkA-p1-status-code-semantics-T7
...
- **Task name:** Pre-commit + make test in both repos
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T6
...
Run pre-commit run --all-files in both repos. Run make test in both repos. Fix any issues.

Plan reference: `plan:m365-cps-trkA-p1-status-code-semantics:T7`. Branch: `feat/m365-cps-trkA`.
...
1. Per parent plan requirements: R1, R8
...
# 3. Validate
cd $M365_ROOT && pre-commit run --all-files && make test && cd $UCP_ROOT && pre-commit run --all-files && make test
...
git add <files>
git commit -m "plan:m365-cps-trkA-p1-status-code-semantics:T7 — Pre-commit + make test in both repos"
```
...
- Validation command exits 0: `cd $M365_ROOT && pre-commit run --all-files && make test && cd $UCP_ROOT && pre-commit run --all-files && make test`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkA-p1-status-code-semantics:T7`
...
- **Validation result:** `cd $M365_ROOT && pre-commit run --all-files && make test && cd $UCP_ROOT && pre-commit run --all-files && make test` exits 0
...
- Reference `plan:m365-cps-trkA-p1-status-code-semantics:T7` in commit message
