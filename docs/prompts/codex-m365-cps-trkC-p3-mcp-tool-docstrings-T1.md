# Codex Execution Prompt — plan:m365-cps-trkC-p3-mcp-tool-docstrings:T1

## Governance Lock (Read FIRST)

- `AGENTS.md` / `CLAUDE.md` (M365 repo and UCP repo)
- `docs/NORTH_STAR.md` (UCP) / `Operations/NORTHSTAR.md` (M365)
- `plans/m365-cps-trkC-p3-mcp-tool-docstrings/m365-cps-trkC-p3-mcp-tool-docstrings.md` (parent phase plan)
...
- **Plan:** `plan:m365-cps-trkC-p3-mcp-tool-docstrings`
- **Parent track:** `plan:m365-cps-trkC-truth-in-advertising`
- **Master plan:** `plan:m365-capability-pack-surface-remediation`
- **Task:** `T1` — Audit each m365_* MCP tool docstring
- **Branch:** `feat/m365-cps-trkC`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkC-p3-mcp-tool-docstrings-T1
...
- **Task name:** Audit each m365_* MCP tool docstring
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** none
...
For each of 23+ m365_* tools in src/ucp/m365_tools.py: list documented actions; cross-reference with registry + alias table; produce audit report.

Plan reference: `plan:m365-cps-trkC-p3-mcp-tool-docstrings:T1`. Branch: `feat/m365-cps-trkC`.
...
# 3. Validate
cd $UCP_ROOT && .venv/bin/python scripts/audit_m365_docstrings.py > artifacts/diagnostics/m365_mcp_docstring_audit.json
...
git add <files>
git commit -m "plan:m365-cps-trkC-p3-mcp-tool-docstrings:T1 — Audit each m365_* MCP tool docstring"
```
...
- Validation command exits 0: `cd $UCP_ROOT && .venv/bin/python scripts/audit_m365_docstrings.py > artifacts/diagnostics/m365_mcp_docstring_audit.json`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkC-p3-mcp-tool-docstrings:T1`
...
- **Validation result:** `cd $UCP_ROOT && .venv/bin/python scripts/audit_m365_docstrings.py > artifacts/diagnostics/m365_mcp_docstring_audit.json` exits 0
...
- Reference `plan:m365-cps-trkC-p3-mcp-tool-docstrings:T1` in commit message
