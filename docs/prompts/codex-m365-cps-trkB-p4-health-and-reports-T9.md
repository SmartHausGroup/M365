# Codex Execution Prompt — plan:m365-cps-trkB-p4-health-and-reports:T9
...
- **Task:** `T9` — Add graph.reports.onedrive_usage registry entry
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p4-health-and-reports-T9
...
- **Task name:** Add graph.reports.onedrive_usage registry entry
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T8
...
Add ActionSpec for graph.reports.onedrive_usage (workload reports; auth_modes ['app_only_secret', 'app_only_certificate']; scopes ['Reports.Read.All']; risk low; rw read) to READ_ONLY_REGISTRY in src/m365_runtime/graph/registry.py. Endpoint per parent plan B-action spec table.

Plan reference: `plan:m365-cps-trkB-p4-health-and-reports:T9`. Branch: `feat/m365-cps-trkB`.
...
- Cross-task changes (this prompt is scoped to T9 only)
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import get_action; spec = get_action('graph.reports.onedrive_usage'); assert spec.workload == 'reports'; print('OK')"
...
git add <files>
git commit -m "plan:m365-cps-trkB-p4-health-and-reports:T9 — Add graph.reports.onedrive_usage registry entry"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import get_action; spec = get_action('graph.reports.onedrive_usage'); assert spec.workload == 'reports'; print('OK')"`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p4-health-and-reports:T9`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import get_action; spec = get_action('graph.reports.onedrive_usage'); assert spec.workload == 'reports'; print('OK')"` exits 0
...
- Reference `plan:m365-cps-trkB-p4-health-and-reports:T9` in commit message
