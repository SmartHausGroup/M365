# Codex Execution Prompt — plan:m365-cps-trkC-p2-agents-yaml-schema:T4
...
- **Task:** `T4` — Update execute_m365_action to surface not_yet_implemented
- **Branch:** `feat/m365-cps-trkC`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkC-p2-agents-yaml-schema-T4
...
- **Task name:** Update execute_m365_action to surface not_yet_implemented
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T3
...
If coverage_status == 'planned', short-circuit: return status_class='not_yet_implemented' without calling runtime.

Plan reference: `plan:m365-cps-trkC-p2-agents-yaml-schema:T4`. Branch: `feat/m365-cps-trkC`.
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python -m pytest tests/test_m365_pack_client.py -k not_yet_implemented -x
...
git add <files>
git commit -m "plan:m365-cps-trkC-p2-agents-yaml-schema:T4 — Update execute_m365_action to surface not_yet_implemented"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python -m pytest tests/test_m365_pack_client.py -k not_yet_implemented -x`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkC-p2-agents-yaml-schema:T4`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python -m pytest tests/test_m365_pack_client.py -k not_yet_implemented -x` exits 0
...
- Reference `plan:m365-cps-trkC-p2-agents-yaml-schema:T4` in commit message
