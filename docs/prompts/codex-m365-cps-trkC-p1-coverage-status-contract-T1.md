# Codex Execution Prompt — plan:m365-cps-trkC-p1-coverage-status-contract:T1
...
- **Task:** `T1` — Define coverage_status enum + JSON schema
- **Branch:** `feat/m365-cps-trkC`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkC-p1-coverage-status-contract-T1
...
- **Task name:** Define coverage_status enum + JSON schema
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** none
...
Define enum {implemented, aliased, planned, deprecated}. JSON schema for coverage_status field on agent action entries.

Plan reference: `plan:m365-cps-trkC-p1-coverage-status-contract:T1`. Branch: `feat/m365-cps-trkC`.
...
# 3. Validate
test -f $M365_ROOT/docs/contracts/m365_coverage_status.json
...
git add <files>
git commit -m "plan:m365-cps-trkC-p1-coverage-status-contract:T1 — Define coverage_status enum + JSON schema"
```
...
- Validation command exits 0: `test -f $M365_ROOT/docs/contracts/m365_coverage_status.json`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkC-p1-coverage-status-contract:T1`
...
- **Validation result:** `test -f $M365_ROOT/docs/contracts/m365_coverage_status.json` exits 0
...
- Reference `plan:m365-cps-trkC-p1-coverage-status-contract:T1` in commit message
