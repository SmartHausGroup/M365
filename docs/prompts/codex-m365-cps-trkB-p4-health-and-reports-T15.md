# Codex Execution Prompt — plan:m365-cps-trkB-p4-health-and-reports:T15
...
- **Task:** `T15` — Commit (no push)
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p4-health-and-reports-T15
...
Commit format 'plan:m365-cps-trkB-p4: T1-T14 <parent name>'.

Plan reference: `plan:m365-cps-trkB-p4-health-and-reports:T15`. Branch: `feat/m365-cps-trkB`.
...
- Cross-task changes (this prompt is scoped to T15 only)
...
# 3. Validate
cd $UCP_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p4' && cd $M365_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p4'
...
git add <files>
git commit -m "plan:m365-cps-trkB-p4-health-and-reports:T15 — Commit (no push)"
```
...
- Validation command exits 0: `cd $UCP_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p4' && cd $M365_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p4'`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p4-health-and-reports:T15`
...
- **Validation result:** `cd $UCP_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p4' && cd $M365_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p4'` exits 0
...
- Reference `plan:m365-cps-trkB-p4-health-and-reports:T15` in commit message
