# Codex Execution Prompt — plan:m365-cps-trkA-p2-inventory-tool:T5
...
- **Task:** `T5` — MCP tool unit tests
- **Branch:** `feat/m365-cps-trkA`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkA-p2-inventory-tool-T5
...
Add test_m365_inventory_tool_shape in tests/test_m365_integration.py validating UCP mcp tool returns the contract.

Plan reference: `plan:m365-cps-trkA-p2-inventory-tool:T5`. Branch: `feat/m365-cps-trkA`.
...
# 3. Validate
cd $UCP_ROOT && .venv/bin/python -m pytest tests/test_m365_integration.py -k inventory_tool -x
...
git add <files>
git commit -m "plan:m365-cps-trkA-p2-inventory-tool:T5 — MCP tool unit tests"
```
...
- Validation command exits 0: `cd $UCP_ROOT && .venv/bin/python -m pytest tests/test_m365_integration.py -k inventory_tool -x`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkA-p2-inventory-tool:T5`
...
- **Validation result:** `cd $UCP_ROOT && .venv/bin/python -m pytest tests/test_m365_integration.py -k inventory_tool -x` exits 0
...
- Reference `plan:m365-cps-trkA-p2-inventory-tool:T5` in commit message
