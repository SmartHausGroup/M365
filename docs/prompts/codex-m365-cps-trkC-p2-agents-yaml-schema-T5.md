# Codex Execution Prompt — plan:m365-cps-trkC-p2-agents-yaml-schema:T5
...
- **Task:** `T5` — Add unit tests
- **Branch:** `feat/m365-cps-trkC`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkC-p2-agents-yaml-schema-T5
...
tests/test_agents_yaml_coverage.py + tests/test_m365_pack_client.py with planned-action behavior.

Plan reference: `plan:m365-cps-trkC-p2-agents-yaml-schema:T5`. Branch: `feat/m365-cps-trkC`.
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python -m pytest -k coverage -x
...
git add <files>
git commit -m "plan:m365-cps-trkC-p2-agents-yaml-schema:T5 — Add unit tests"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python -m pytest -k coverage -x`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkC-p2-agents-yaml-schema:T5`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python -m pytest -k coverage -x` exits 0
...
- Reference `plan:m365-cps-trkC-p2-agents-yaml-schema:T5` in commit message
