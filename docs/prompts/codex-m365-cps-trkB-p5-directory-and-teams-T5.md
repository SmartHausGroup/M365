# Codex Execution Prompt — plan:m365-cps-trkB-p5-directory-and-teams:T5

## Governance Lock (Read FIRST)

- `AGENTS.md` / `CLAUDE.md` (M365 repo and UCP repo)
- `docs/NORTH_STAR.md` (UCP) / `Operations/NORTHSTAR.md` (M365)
- `plans/m365-cps-trkB-p5-directory-and-teams/m365-cps-trkB-p5-directory-and-teams.md` (parent phase plan)
...
- **Plan:** `plan:m365-cps-trkB-p5-directory-and-teams`
- **Parent track:** `plan:m365-cps-trkB-coverage`
- **Master plan:** `plan:m365-capability-pack-surface-remediation`
- **Task:** `T5` — Add graph.channels.list registry entry
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p5-directory-and-teams-T5
...
- **Task name:** Add graph.channels.list registry entry
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T4
...
Add ActionSpec for graph.channels.list (workload teams; auth_modes ['auth_code_pkce', 'app_only_secret', 'app_only_certificate']; scopes ['Channel.ReadBasic.All']; risk low; rw read) to READ_ONLY_REGISTRY in src/m365_runtime/graph/registry.py. Endpoint per parent plan B-action spec table.

Plan reference: `plan:m365-cps-trkB-p5-directory-and-teams:T5`. Branch: `feat/m365-cps-trkB`.
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import get_action; spec = get_action('graph.channels.list'); assert spec.workload == 'teams'; print('OK')"
...
git add <files>
git commit -m "plan:m365-cps-trkB-p5-directory-and-teams:T5 — Add graph.channels.list registry entry"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import get_action; spec = get_action('graph.channels.list'); assert spec.workload == 'teams'; print('OK')"`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p5-directory-and-teams:T5`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import get_action; spec = get_action('graph.channels.list'); assert spec.workload == 'teams'; print('OK')"` exits 0
...
- Reference `plan:m365-cps-trkB-p5-directory-and-teams:T5` in commit message
