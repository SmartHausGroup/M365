# Codex Execution Prompt — plan:m365-cps-trkB-p5-directory-and-teams:T8
...
- **Task:** `T8` — Update YAML registry mirror
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p5-directory-and-teams-T8
...
- **Task name:** Update YAML registry mirror
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T7
...
Update registry/action_registry.yaml with corresponding stanzas for the new graph.* actions.

Plan reference: `plan:m365-cps-trkB-p5-directory-and-teams:T8`. Branch: `feat/m365-cps-trkB`.
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python -c "import yaml; data=yaml.safe_load(open('registry/action_registry.yaml')); assert len(data['actions']) > 11"
...
git add <files>
git commit -m "plan:m365-cps-trkB-p5-directory-and-teams:T8 — Update YAML registry mirror"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python -c "import yaml; data=yaml.safe_load(open('registry/action_registry.yaml')); assert len(data['actions']) > 11"`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p5-directory-and-teams:T8`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python -c "import yaml; data=yaml.safe_load(open('registry/action_registry.yaml')); assert len(data['actions']) > 11"` exits 0
...
- Reference `plan:m365-cps-trkB-p5-directory-and-teams:T8` in commit message
