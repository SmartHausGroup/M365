# Codex Execution Prompt — plan:m365-cps-trkB-p2-calendar-reads:T11
...
- **Task:** `T11` — Commit (no push)
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p2-calendar-reads-T11
...
Commit format 'plan:m365-cps-trkB-p2: T1-T10 <parent name>'.

Plan reference: `plan:m365-cps-trkB-p2-calendar-reads:T11`. Branch: `feat/m365-cps-trkB`.
...
# 3. Validate
cd $UCP_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p2' && cd $M365_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p2'
...
git add <files>
git commit -m "plan:m365-cps-trkB-p2-calendar-reads:T11 — Commit (no push)"
```
...
- Validation command exits 0: `cd $UCP_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p2' && cd $M365_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p2'`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p2-calendar-reads:T11`
...
- **Validation result:** `cd $UCP_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p2' && cd $M365_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p2'` exits 0
...
- Reference `plan:m365-cps-trkB-p2-calendar-reads:T11` in commit message
