# Codex Execution Prompt — plan:m365-cps-trkB-p6-auth-mode-tiers:T7
...
- **Task:** `T7` — Live tenant smoke (each tier)
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p6-auth-mode-tiers-T7
...
- **Task name:** Live tenant smoke (each tier)
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T6
...
Auth at each tier; verify scope set returned by Graph matches expected.

Plan reference: `plan:m365-cps-trkB-p6-auth-mode-tiers:T7`. Branch: `feat/m365-cps-trkB`.
...
# 3. Validate
cd $UCP_ROOT && .venv/bin/python tests/integration/m365_auth_tiers_live.py
...
git add <files>
git commit -m "plan:m365-cps-trkB-p6-auth-mode-tiers:T7 — Live tenant smoke (each tier)"
```
...
- Validation command exits 0: `cd $UCP_ROOT && .venv/bin/python tests/integration/m365_auth_tiers_live.py`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p6-auth-mode-tiers:T7`
...
- **Validation result:** `cd $UCP_ROOT && .venv/bin/python tests/integration/m365_auth_tiers_live.py` exits 0
...
- Reference `plan:m365-cps-trkB-p6-auth-mode-tiers:T7` in commit message
