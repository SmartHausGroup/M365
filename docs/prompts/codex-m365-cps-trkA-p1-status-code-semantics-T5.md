# Codex Execution Prompt — plan:m365-cps-trkA-p1-status-code-semantics:T5
...
- **Task:** `T5` — Add M365 unit tests for unknown_action
- **Branch:** `feat/m365-cps-trkA`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkA-p1-status-code-semantics-T5
...
- **Task name:** Add M365 unit tests for unknown_action
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T4
...
Add test_unknown_action_returns_unknown_action_status in tests/test_m365_pack_client.py. Update any existing tests that asserted mutation_fence on unknown actions.

Plan reference: `plan:m365-cps-trkA-p1-status-code-semantics:T5`. Branch: `feat/m365-cps-trkA`.
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python -m pytest tests/test_m365_pack_client.py -k unknown_action -x
...
git add <files>
git commit -m "plan:m365-cps-trkA-p1-status-code-semantics:T5 — Add M365 unit tests for unknown_action"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python -m pytest tests/test_m365_pack_client.py -k unknown_action -x`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkA-p1-status-code-semantics:T5`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python -m pytest tests/test_m365_pack_client.py -k unknown_action -x` exits 0
...
- Reference `plan:m365-cps-trkA-p1-status-code-semantics:T5` in commit message
