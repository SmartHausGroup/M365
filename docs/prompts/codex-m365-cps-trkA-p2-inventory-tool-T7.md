# Codex Execution Prompt — plan:m365-cps-trkA-p2-inventory-tool:T7
...
- **Task:** `T7` — Commit child phases (no push)
- **Branch:** `feat/m365-cps-trkA`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkA-p2-inventory-tool-T7
...
Commit format 'plan:m365-cps-trkA-p2: T1-T6 inventory tool'.

Plan reference: `plan:m365-cps-trkA-p2-inventory-tool:T7`. Branch: `feat/m365-cps-trkA`.
...
# 3. Validate
cd $UCP_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkA-p2' && cd $M365_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkA-p2'
...
git add <files>
git commit -m "plan:m365-cps-trkA-p2-inventory-tool:T7 — Commit child phases (no push)"
```
...
- Validation command exits 0: `cd $UCP_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkA-p2' && cd $M365_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkA-p2'`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkA-p2-inventory-tool:T7`
...
- **Validation result:** `cd $UCP_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkA-p2' && cd $M365_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkA-p2'` exits 0
...
- Reference `plan:m365-cps-trkA-p2-inventory-tool:T7` in commit message
