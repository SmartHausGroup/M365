# Codex Execution Prompt — plan:m365-cps-trkC-p5-end-to-end:T4
...
- **Task:** `T4` — Update CHANGELOG / EXECUTION_PLAN / ACTION_LOG
- **Branch:** `feat/m365-cps-trkC`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkC-p5-end-to-end-T4
...
Track C completion entries.

Plan reference: `plan:m365-cps-trkC-p5-end-to-end:T4`. Branch: `feat/m365-cps-trkC`.
...
# 3. Validate
cd $UCP_ROOT && git diff --stat docs/platform/EXECUTION_PLAN.md && cd $M365_ROOT && git diff --stat Operations/EXECUTION_PLAN.md
...
git add <files>
git commit -m "plan:m365-cps-trkC-p5-end-to-end:T4 — Update CHANGELOG / EXECUTION_PLAN / ACTION_LOG"
```
...
- Validation command exits 0: `cd $UCP_ROOT && git diff --stat docs/platform/EXECUTION_PLAN.md && cd $M365_ROOT && git diff --stat Operations/EXECUTION_PLAN.md`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkC-p5-end-to-end:T4`
...
- **Validation result:** `cd $UCP_ROOT && git diff --stat docs/platform/EXECUTION_PLAN.md && cd $M365_ROOT && git diff --stat Operations/EXECUTION_PLAN.md` exits 0
...
- Reference `plan:m365-cps-trkC-p5-end-to-end:T4` in commit message
