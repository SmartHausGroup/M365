# Codex Execution Prompt — plan:m365-cps-trkC-p2-agents-yaml-schema:T1
...
- **Task:** `T1` — Update agents.yaml schema
- **Branch:** `feat/m365-cps-trkC`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkC-p2-agents-yaml-schema-T1
...
- **Task name:** Update agents.yaml schema
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** none
...
Modify schema to allow allowed_actions entries to be either str or {action: str, coverage_status: enum}.

Plan reference: `plan:m365-cps-trkC-p2-agents-yaml-schema:T1`. Branch: `feat/m365-cps-trkC`.
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python -c "import yaml; data=yaml.safe_load(open('registry/agents.yaml')); print('valid')"
...
git add <files>
git commit -m "plan:m365-cps-trkC-p2-agents-yaml-schema:T1 — Update agents.yaml schema"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python -c "import yaml; data=yaml.safe_load(open('registry/agents.yaml')); print('valid')"`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkC-p2-agents-yaml-schema:T1`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python -c "import yaml; data=yaml.safe_load(open('registry/agents.yaml')); print('valid')"` exits 0
...
- Reference `plan:m365-cps-trkC-p2-agents-yaml-schema:T1` in commit message
