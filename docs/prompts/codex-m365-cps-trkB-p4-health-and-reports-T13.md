# Codex Execution Prompt — plan:m365-cps-trkB-p4-health-and-reports:T13
...
- **Task:** `T13` — UCP routing unit tests
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p4-health-and-reports-T13
...
Plan reference: `plan:m365-cps-trkB-p4-health-and-reports:T13`. Branch: `feat/m365-cps-trkB`.
...
- Cross-task changes (this prompt is scoped to T13 only)
...
# 3. Validate
cd $UCP_ROOT && .venv/bin/python -m pytest tests/test_m365_integration.py -k 'p4' -x
...
git add <files>
git commit -m "plan:m365-cps-trkB-p4-health-and-reports:T13 — UCP routing unit tests"
```
...
- Validation command exits 0: `cd $UCP_ROOT && .venv/bin/python -m pytest tests/test_m365_integration.py -k 'p4' -x`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p4-health-and-reports:T13`
...
- **Validation result:** `cd $UCP_ROOT && .venv/bin/python -m pytest tests/test_m365_integration.py -k 'p4' -x` exits 0
...
- Reference `plan:m365-cps-trkB-p4-health-and-reports:T13` in commit message
