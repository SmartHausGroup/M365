# Codex Execution Prompt — plan:m365-cps-trkA-p3-preflight-intersection:T5
...
- **Task:** `T5` — Unit tests both repos
- **Branch:** `feat/m365-cps-trkA`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkA-p3-preflight-intersection-T5
...
M365: test_compute_intersection coverage. UCP: test_auth_check_preflight_passthrough.

Plan reference: `plan:m365-cps-trkA-p3-preflight-intersection:T5`. Branch: `feat/m365-cps-trkA`.
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python -m pytest -k preflight -x && cd $UCP_ROOT && .venv/bin/python -m pytest -k preflight -x
...
git add <files>
git commit -m "plan:m365-cps-trkA-p3-preflight-intersection:T5 — Unit tests both repos"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python -m pytest -k preflight -x && cd $UCP_ROOT && .venv/bin/python -m pytest -k preflight -x`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkA-p3-preflight-intersection:T5`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python -m pytest -k preflight -x && cd $UCP_ROOT && .venv/bin/python -m pytest -k preflight -x` exits 0
...
- Reference `plan:m365-cps-trkA-p3-preflight-intersection:T5` in commit message
