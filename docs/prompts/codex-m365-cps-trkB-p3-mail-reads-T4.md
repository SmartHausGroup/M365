# Codex Execution Prompt — plan:m365-cps-trkB-p3-mail-reads:T4

## Governance Lock (Read FIRST)

- `AGENTS.md` / `CLAUDE.md` (M365 repo and UCP repo)
- `docs/NORTH_STAR.md` (UCP) / `Operations/NORTHSTAR.md` (M365)
- `plans/m365-cps-trkB-p3-mail-reads/m365-cps-trkB-p3-mail-reads.md` (parent phase plan)
...
- **Plan:** `plan:m365-cps-trkB-p3-mail-reads`
- **Parent track:** `plan:m365-cps-trkB-coverage`
- **Master plan:** `plan:m365-capability-pack-surface-remediation`
- **Task:** `T4` — Add graph.mail.attachments registry entry
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p3-mail-reads-T4
...
- **Task name:** Add graph.mail.attachments registry entry
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T3
...
Add ActionSpec for graph.mail.attachments (workload exchange; auth_modes ['auth_code_pkce', 'device_code']; scopes ['Mail.Read']; risk low; rw read) to READ_ONLY_REGISTRY in src/m365_runtime/graph/registry.py. Endpoint per parent plan B-action spec table.

Plan reference: `plan:m365-cps-trkB-p3-mail-reads:T4`. Branch: `feat/m365-cps-trkB`.
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import get_action; spec = get_action('graph.mail.attachments'); assert spec.workload == 'exchange'; print('OK')"
...
git add <files>
git commit -m "plan:m365-cps-trkB-p3-mail-reads:T4 — Add graph.mail.attachments registry entry"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import get_action; spec = get_action('graph.mail.attachments'); assert spec.workload == 'exchange'; print('OK')"`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p3-mail-reads:T4`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import get_action; spec = get_action('graph.mail.attachments'); assert spec.workload == 'exchange'; print('OK')"` exits 0
...
- Reference `plan:m365-cps-trkB-p3-mail-reads:T4` in commit message
