# Codex Execution Prompt — plan:m365-cps-trkA-p1-status-code-semantics:T3
...
- **Task:** `T3` — Align actions.py callsite
- **Branch:** `feat/m365-cps-trkA`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkA-p1-status-code-semantics-T3
...
- **Task name:** Align actions.py callsite
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T2
...
Verify ActionInvocation.status_class plumbs the new 'unknown_action' value through unchanged. No code change expected; this is a verification + assertion task.

Plan reference: `plan:m365-cps-trkA-p1-status-code-semantics:T3`. Branch: `feat/m365-cps-trkA`.
...
# 3. Validate
cd $M365_ROOT && grep -n 'unknown_action' src/m365_runtime/graph/actions.py || echo 'no remap callsites - OK'
...
git add <files>
git commit -m "plan:m365-cps-trkA-p1-status-code-semantics:T3 — Align actions.py callsite"
```
...
- Validation command exits 0: `cd $M365_ROOT && grep -n 'unknown_action' src/m365_runtime/graph/actions.py || echo 'no remap callsites - OK'`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkA-p1-status-code-semantics:T3`
...
- **Validation result:** `cd $M365_ROOT && grep -n 'unknown_action' src/m365_runtime/graph/actions.py || echo 'no remap callsites - OK'` exits 0
...
- Reference `plan:m365-cps-trkA-p1-status-code-semantics:T3` in commit message
