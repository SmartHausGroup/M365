# Codex Execution Prompt — plan:m365-cps-trkA-p3-preflight-intersection:T4
...
- **Task:** `T4` — Surface in UCP m365_auth_check
- **Branch:** `feat/m365-cps-trkA`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkA-p3-preflight-intersection-T4
...
- **Task name:** Surface in UCP m365_auth_check
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T3
...
Update UCP m365_auth_check tool to pass through the preflight intersection in its return.

Plan reference: `plan:m365-cps-trkA-p3-preflight-intersection:T4`. Branch: `feat/m365-cps-trkA`.
...
# 3. Validate
cd $UCP_ROOT && .venv/bin/python -m pytest tests/test_m365_integration.py -k auth_check_preflight -x
...
git add <files>
git commit -m "plan:m365-cps-trkA-p3-preflight-intersection:T4 — Surface in UCP m365_auth_check"
```
...
- Validation command exits 0: `cd $UCP_ROOT && .venv/bin/python -m pytest tests/test_m365_integration.py -k auth_check_preflight -x`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkA-p3-preflight-intersection:T4`
...
- **Validation result:** `cd $UCP_ROOT && .venv/bin/python -m pytest tests/test_m365_integration.py -k auth_check_preflight -x` exits 0
...
- Reference `plan:m365-cps-trkA-p3-preflight-intersection:T4` in commit message
