# Codex Execution Prompt — plan:m365-cps-trkC-p2-agents-yaml-schema:T2
...
- **Task:** `T2` — Migrate existing agents.yaml entries with coverage_status
- **Branch:** `feat/m365-cps-trkC`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkC-p2-agents-yaml-schema-T2
...
- **Task name:** Migrate existing agents.yaml entries with coverage_status
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T1
...
For each allowed_action: check if action_id is in alias table (status=aliased), in registry directly (status=implemented), or neither (status=planned). Migrate entries.

Plan reference: `plan:m365-cps-trkC-p2-agents-yaml-schema:T2`. Branch: `feat/m365-cps-trkC`.
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python scripts/check_agents_yaml_coverage.py
...
git add <files>
git commit -m "plan:m365-cps-trkC-p2-agents-yaml-schema:T2 — Migrate existing agents.yaml entries with coverage_status"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python scripts/check_agents_yaml_coverage.py`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkC-p2-agents-yaml-schema:T2`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python scripts/check_agents_yaml_coverage.py` exits 0
...
- Reference `plan:m365-cps-trkC-p2-agents-yaml-schema:T2` in commit message
