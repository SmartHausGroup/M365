# Codex Execution Prompt — plan:m365-cps-trkB-p5-directory-and-teams:T9
...
- **Task:** `T9` — M365 unit tests
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p5-directory-and-teams-T9
...
Add per-action unit tests in tests/test_m365_pack_client.py covering: registry lookup, admit() success, admit() denial when missing scope.

Plan reference: `plan:m365-cps-trkB-p5-directory-and-teams:T9`. Branch: `feat/m365-cps-trkB`.
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python -m pytest tests/test_m365_pack_client.py -k 'p5' -x
...
git add <files>
git commit -m "plan:m365-cps-trkB-p5-directory-and-teams:T9 — M365 unit tests"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python -m pytest tests/test_m365_pack_client.py -k 'p5' -x`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p5-directory-and-teams:T9`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python -m pytest tests/test_m365_pack_client.py -k 'p5' -x` exits 0
...
- Reference `plan:m365-cps-trkB-p5-directory-and-teams:T9` in commit message
