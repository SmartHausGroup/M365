# Codex Execution Prompt — plan:m365-cps-trkB-p5-directory-and-teams:T1
...
- **Task:** `T1` — Read context for parent phase
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p5-directory-and-teams-T1
...
- **Task name:** Read context for parent phase
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** none
...
Read existing m365_runtime/graph/registry.py, ucp_m365_pack/client.py alias table, registry/action_registry.yaml. Document existing patterns to match.

Plan reference: `plan:m365-cps-trkB-p5-directory-and-teams:T1`. Branch: `feat/m365-cps-trkB`.
...
cat src/ucp_m365_pack/client.py
cat registry/action_registry.yaml

# 2. Make the edits per M+A above (use Read tool first per A1 of CLAUDE.md)

# 3. Validate
test -f $M365_ROOT/src/m365_runtime/graph/registry.py
...
git add <files>
git commit -m "plan:m365-cps-trkB-p5-directory-and-teams:T1 — Read context for parent phase"
```
...
- Validation command exits 0: `test -f $M365_ROOT/src/m365_runtime/graph/registry.py`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p5-directory-and-teams:T1`
...
- **Validation result:** `test -f $M365_ROOT/src/m365_runtime/graph/registry.py` exits 0
...
- Reference `plan:m365-cps-trkB-p5-directory-and-teams:T1` in commit message
