# Codex Execution Prompt — plan:m365-cps-trkA-p2-inventory-tool:T3
...
- **Task:** `T3` — Runtime endpoint unit tests
- **Branch:** `feat/m365-cps-trkA`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkA-p2-inventory-tool-T3
...
- **Task name:** Runtime endpoint unit tests
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T2
...
Add tests/test_m365_inventory_endpoint.py covering: (a) implemented_actions matches READ_ONLY_REGISTRY keys, (b) alias_map matches LEGACY_ACTION_TO_RUNTIME_ACTION, (c) advertised_only correctly diffs agents.yaml.

Plan reference: `plan:m365-cps-trkA-p2-inventory-tool:T3`. Branch: `feat/m365-cps-trkA`.
...
git add <files>
git commit -m "plan:m365-cps-trkA-p2-inventory-tool:T3 — Runtime endpoint unit tests"
```
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkA-p2-inventory-tool:T3`
...
- Reference `plan:m365-cps-trkA-p2-inventory-tool:T3` in commit message
