# Codex Execution Prompt — plan:m365-cps-trkB-p6-auth-mode-tiers:T1
...
- **Task:** `T1` — Define scope tier sets
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p6-auth-mode-tiers-T1
...
- **Task name:** Define scope tier sets
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** none
...
Document the three tiers and their Graph scope sets: read-only={User.Read, Mail.Read, Calendars.Read, Sites.Read.All, Files.Read.All, Team.ReadBasic.All}; standard=read-only + write scopes for Mail/Calendar/Files; admin=standard + Directory.ReadWrite.All, RoleManagement.ReadWrite.Directory.

Plan reference: `plan:m365-cps-trkB-p6-auth-mode-tiers:T1`. Branch: `feat/m365-cps-trkB`.
...
# 3. Validate
test -f $M365_ROOT/docs/contracts/m365_auth_tiers.md
...
git add <files>
git commit -m "plan:m365-cps-trkB-p6-auth-mode-tiers:T1 — Define scope tier sets"
```
...
- Validation command exits 0: `test -f $M365_ROOT/docs/contracts/m365_auth_tiers.md`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p6-auth-mode-tiers:T1`
...
- **Validation result:** `test -f $M365_ROOT/docs/contracts/m365_auth_tiers.md` exits 0
...
- Reference `plan:m365-cps-trkB-p6-auth-mode-tiers:T1` in commit message
