# Codex Execution Prompt — plan:m365-cps-trkB-p1-sharepoint-reads:T11
...
- **Task:** `T11` — UCP routing unit tests
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p1-sharepoint-reads-T11
...
Plan reference: `plan:m365-cps-trkB-p1-sharepoint-reads:T11`. Branch: `feat/m365-cps-trkB`.
...
# 3. Validate
cd $UCP_ROOT && .venv/bin/python -m pytest tests/test_m365_integration.py -k 'p1' -x
...
git add <files>
git commit -m "plan:m365-cps-trkB-p1-sharepoint-reads:T11 — UCP routing unit tests"
```
...
- Validation command exits 0: `cd $UCP_ROOT && .venv/bin/python -m pytest tests/test_m365_integration.py -k 'p1' -x`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p1-sharepoint-reads:T11`
...
- **Validation result:** `cd $UCP_ROOT && .venv/bin/python -m pytest tests/test_m365_integration.py -k 'p1' -x` exits 0
...
- Reference `plan:m365-cps-trkB-p1-sharepoint-reads:T11` in commit message
