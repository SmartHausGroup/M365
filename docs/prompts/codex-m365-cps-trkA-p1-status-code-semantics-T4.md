# Codex Execution Prompt — plan:m365-cps-trkA-p1-status-code-semantics:T4
...
- **Task:** `T4` — Update UCP m365_tools.py to surface unknown_action
- **Branch:** `feat/m365-cps-trkA`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkA-p1-status-code-semantics-T4
...
- **Task name:** Update UCP m365_tools.py to surface unknown_action
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T3
...
In UCP src/ucp/m365_tools.py, ensure the_m365_execute path surfaces status_class == 'unknown_action' without remapping it back to mutation_fence.

Plan reference: `plan:m365-cps-trkA-p1-status-code-semantics:T4`. Branch: `feat/m365-cps-trkA`.
...
# 3. Validate
cd $UCP_ROOT && grep -n 'unknown_action\|mutation_fence' src/ucp/m365_tools.py || true
...
git add <files>
git commit -m "plan:m365-cps-trkA-p1-status-code-semantics:T4 — Update UCP m365_tools.py to surface unknown_action"
```
...
- Validation command exits 0: `cd $UCP_ROOT && grep -n 'unknown_action\|mutation_fence' src/ucp/m365_tools.py || true`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkA-p1-status-code-semantics:T4`
...
- **Validation result:** `cd $UCP_ROOT && grep -n 'unknown_action\|mutation_fence' src/ucp/m365_tools.py || true` exits 0
...
- Reference `plan:m365-cps-trkA-p1-status-code-semantics:T4` in commit message
