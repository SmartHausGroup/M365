# Codex Execution Prompt — plan:m365-cps-trkB-p3-mail-reads:T10
...
- **Task:** `T10` — Commit (no push)
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p3-mail-reads-T10
...
Commit format 'plan:m365-cps-trkB-p3: T1-T9 <parent name>'.

Plan reference: `plan:m365-cps-trkB-p3-mail-reads:T10`. Branch: `feat/m365-cps-trkB`.
...
# 3. Validate
cd $UCP_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p3' && cd $M365_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p3'
...
git add <files>
git commit -m "plan:m365-cps-trkB-p3-mail-reads:T10 — Commit (no push)"
```
...
- Validation command exits 0: `cd $UCP_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p3' && cd $M365_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p3'`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p3-mail-reads:T10`
...
- **Validation result:** `cd $UCP_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p3' && cd $M365_ROOT && git log --oneline -1 | grep 'plan:m365-cps-trkB-p3'` exits 0
...
- Reference `plan:m365-cps-trkB-p3-mail-reads:T10` in commit message
