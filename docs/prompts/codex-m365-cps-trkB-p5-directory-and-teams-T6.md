# Codex Execution Prompt — plan:m365-cps-trkB-p5-directory-and-teams:T6
...
- **Task:** `T6` — Add graph.channels.get registry entry
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p5-directory-and-teams-T6
...
- **Task name:** Add graph.channels.get registry entry
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T5
...
Add ActionSpec for graph.channels.get (workload teams; auth_modes ['auth_code_pkce', 'app_only_secret', 'app_only_certificate']; scopes ['Channel.ReadBasic.All']; risk low; rw read) to READ_ONLY_REGISTRY in src/m365_runtime/graph/registry.py. Endpoint per parent plan B-action spec table.

Plan reference: `plan:m365-cps-trkB-p5-directory-and-teams:T6`. Branch: `feat/m365-cps-trkB`.
...
- Cross-task changes (this prompt is scoped to T6 only)
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import get_action; spec = get_action('graph.channels.get'); assert spec.workload == 'teams'; print('OK')"
...
git add <files>
git commit -m "plan:m365-cps-trkB-p5-directory-and-teams:T6 — Add graph.channels.get registry entry"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import get_action; spec = get_action('graph.channels.get'); assert spec.workload == 'teams'; print('OK')"`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p5-directory-and-teams:T6`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import get_action; spec = get_action('graph.channels.get'); assert spec.workload == 'teams'; print('OK')"` exits 0
...
- Reference `plan:m365-cps-trkB-p5-directory-and-teams:T6` in commit message
