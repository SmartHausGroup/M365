# Codex Execution Prompt — plan:m365-cps-trkA-p4-end-to-end:T2
...
- **Task:** `T2` — Verify status semantics on real failed calls
- **Branch:** `feat/m365-cps-trkA`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkA-p4-end-to-end-T2
...
- **Task name:** Verify status semantics on real failed calls
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T1
...
Trigger known unknown actions against the live runtime. Verify response status_class == 'unknown_action'.

Plan reference: `plan:m365-cps-trkA-p4-end-to-end:T2`. Branch: `feat/m365-cps-trkA`.
...
# 3. Validate
cd $UCP_ROOT && .venv/bin/python tests/integration/m365_unknown_action_live.py
...
git add <files>
git commit -m "plan:m365-cps-trkA-p4-end-to-end:T2 — Verify status semantics on real failed calls"
```
...
- Validation command exits 0: `cd $UCP_ROOT && .venv/bin/python tests/integration/m365_unknown_action_live.py`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkA-p4-end-to-end:T2`
...
- **Validation result:** `cd $UCP_ROOT && .venv/bin/python tests/integration/m365_unknown_action_live.py` exits 0
...
- Reference `plan:m365-cps-trkA-p4-end-to-end:T2` in commit message
