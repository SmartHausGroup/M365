# Codex Execution Prompt — plan:m365-cps-trkA-p1-status-code-semantics:T1

## Governance Lock (Read FIRST)

- `AGENTS.md` / `CLAUDE.md` (M365 repo and UCP repo)
- `docs/NORTH_STAR.md` (UCP) / `Operations/NORTHSTAR.md` (M365)
- `plans/m365-cps-trkA-p1-status-code-semantics/m365-cps-trkA-p1-status-code-semantics.md` (parent phase plan)
...
- **Plan:** `plan:m365-cps-trkA-p1-status-code-semantics`
- **Parent track:** `plan:m365-cps-trkA-observability`
- **Master plan:** `plan:m365-capability-pack-surface-remediation`
- **Task:** `T1` — Read context for status code semantics
- **Branch:** `feat/m365-cps-trkA`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkA-p1-status-code-semantics-T1
...
- **Task name:** Read context for status code semantics
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** none
...
Read M365 src/m365_runtime/graph/registry.py, actions.py end-to-end. Read UCP src/ucp/m365_tools.py. Document current _denial_to_status mapping behavior in commit notes.

Plan reference: `plan:m365-cps-trkA-p1-status-code-semantics:T1`. Branch: `feat/m365-cps-trkA`.
...
1. Per parent plan requirements: R1
...
```bash
# 1. Read context
cat src/m365_runtime/graph/registry.py
cat src/m365_runtime/graph/actions.py
cat src/ucp/m365_tools.py

# 2. Make the edits per M+A above (use Read tool first per A1 of CLAUDE.md)

# 3. Validate
test -f $M365_ROOT/src/m365_runtime/graph/registry.py && test -f $M365_ROOT/src/m365_runtime/graph/actions.py && test -f $UCP_ROOT/src/ucp/m365_tools.py
...
git add <files>
git commit -m "plan:m365-cps-trkA-p1-status-code-semantics:T1 — Read context for status code semantics"
```
...
- Validation command exits 0: `test -f $M365_ROOT/src/m365_runtime/graph/registry.py && test -f $M365_ROOT/src/m365_runtime/graph/actions.py && test -f $UCP_ROOT/src/ucp/m365_tools.py`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkA-p1-status-code-semantics:T1`
...
- **Validation result:** `test -f $M365_ROOT/src/m365_runtime/graph/registry.py && test -f $M365_ROOT/src/m365_runtime/graph/actions.py && test -f $UCP_ROOT/src/ucp/m365_tools.py` exits 0
...
- Reference `plan:m365-cps-trkA-p1-status-code-semantics:T1` in commit message
