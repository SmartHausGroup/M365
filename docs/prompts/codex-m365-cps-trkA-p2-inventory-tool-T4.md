# Codex Execution Prompt — plan:m365-cps-trkA-p2-inventory-tool:T4
...
- **Task:** `T4` — Add UCP m365_inventory MCP tool
- **Branch:** `feat/m365-cps-trkA`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkA-p2-inventory-tool-T4
...
- **Task name:** Add UCP m365_inventory MCP tool
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T3
...
Add @mcp.tool() m365_inventory in src/ucp/m365_tools.py that fetches the runtime /v1/inventory and returns the parsed JSON. Include shape validation against the contract schema.

Plan reference: `plan:m365-cps-trkA-p2-inventory-tool:T4`. Branch: `feat/m365-cps-trkA`.
...
# 3. Validate
cd $UCP_ROOT && .venv/bin/python -c "from ucp.m365_tools import m365_inventory; print('importable')"
...
git add <files>
git commit -m "plan:m365-cps-trkA-p2-inventory-tool:T4 — Add UCP m365_inventory MCP tool"
```
...
- Validation command exits 0: `cd $UCP_ROOT && .venv/bin/python -c "from ucp.m365_tools import m365_inventory; print('importable')"`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkA-p2-inventory-tool:T4`
...
- **Validation result:** `cd $UCP_ROOT && .venv/bin/python -c "from ucp.m365_tools import m365_inventory; print('importable')"` exits 0
...
- Reference `plan:m365-cps-trkA-p2-inventory-tool:T4` in commit message
