# Codex Execution Prompt — plan:m365-cps-trkA-p3-preflight-intersection:T1
...
- **Task:** `T1` — Define intersection algorithm
- **Branch:** `feat/m365-cps-trkA`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkA-p3-preflight-intersection-T1
...
- **Task name:** Define intersection algorithm
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** none
...
Document the algorithm for token-vs-registry intersection: invokable = {a in REGISTRY : session.auth_mode in a.auth_modes AND a.scopes <= session.granted_scopes}. blocked_by_auth_mode = REGISTRY \ invokable filtered by auth_mode. blocked_by_scopes = REGISTRY \ invokable filtered by scopes.

Plan reference: `plan:m365-cps-trkA-p3-preflight-intersection:T1`. Branch: `feat/m365-cps-trkA`.
...
# 3. Validate
test -f $M365_ROOT/docs/contracts/m365_preflight_intersection.md || echo 'inline-acceptable'
...
git add <files>
git commit -m "plan:m365-cps-trkA-p3-preflight-intersection:T1 — Define intersection algorithm"
```
...
- Validation command exits 0: `test -f $M365_ROOT/docs/contracts/m365_preflight_intersection.md || echo 'inline-acceptable'`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkA-p3-preflight-intersection:T1`
...
- **Validation result:** `test -f $M365_ROOT/docs/contracts/m365_preflight_intersection.md || echo 'inline-acceptable'` exits 0
...
- Reference `plan:m365-cps-trkA-p3-preflight-intersection:T1` in commit message
