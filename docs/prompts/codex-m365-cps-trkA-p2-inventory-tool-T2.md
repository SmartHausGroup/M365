# Codex Execution Prompt — plan:m365-cps-trkA-p2-inventory-tool:T2
...
- **Task:** `T2` — Implement runtime /v1/inventory endpoint
- **Branch:** `feat/m365-cps-trkA`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkA-p2-inventory-tool-T2
...
- **Task name:** Implement runtime /v1/inventory endpoint
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T1
...
Add /v1/inventory endpoint in m365_runtime that returns the inventory contract: list READ_ONLY_REGISTRY actions, the alias map (loaded from ucp_m365_pack.client), and the advertised-but-unimplemented set (agents.yaml allowed_actions minus alias keys minus registry keys).

Plan reference: `plan:m365-cps-trkA-p2-inventory-tool:T2`. Branch: `feat/m365-cps-trkA`.
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python -m pytest tests/test_m365_inventory_endpoint.py -x
...
git add <files>
git commit -m "plan:m365-cps-trkA-p2-inventory-tool:T2 — Implement runtime /v1/inventory endpoint"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python -m pytest tests/test_m365_inventory_endpoint.py -x`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkA-p2-inventory-tool:T2`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python -m pytest tests/test_m365_inventory_endpoint.py -x` exits 0
...
- Reference `plan:m365-cps-trkA-p2-inventory-tool:T2` in commit message
