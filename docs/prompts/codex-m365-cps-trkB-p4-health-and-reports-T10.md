# Codex Execution Prompt — plan:m365-cps-trkB-p4-health-and-reports:T10
...
- **Task:** `T10` — Update legacy alias table
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p4-health-and-reports-T10
...
- **Task name:** Update legacy alias table
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T9
...
Update LEGACY_ACTION_TO_RUNTIME_ACTION in ucp_m365_pack/client.py with legacy_name -> graph.X.Y mappings for the new actions.

Plan reference: `plan:m365-cps-trkB-p4-health-and-reports:T10`. Branch: `feat/m365-cps-trkB`.
...
- Cross-task changes (this prompt is scoped to T10 only)
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python -c "from ucp_m365_pack.client import map_legacy_action_to_runtime, LEGACY_ACTION_TO_RUNTIME_ACTION; assert len(LEGACY_ACTION_TO_RUNTIME_ACTION) > 22"
...
git add <files>
git commit -m "plan:m365-cps-trkB-p4-health-and-reports:T10 — Update legacy alias table"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python -c "from ucp_m365_pack.client import map_legacy_action_to_runtime, LEGACY_ACTION_TO_RUNTIME_ACTION; assert len(LEGACY_ACTION_TO_RUNTIME_ACTION) > 22"`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p4-health-and-reports:T10`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python -c "from ucp_m365_pack.client import map_legacy_action_to_runtime, LEGACY_ACTION_TO_RUNTIME_ACTION; assert len(LEGACY_ACTION_TO_RUNTIME_ACTION) > 22"` exits 0
...
- Reference `plan:m365-cps-trkB-p4-health-and-reports:T10` in commit message
