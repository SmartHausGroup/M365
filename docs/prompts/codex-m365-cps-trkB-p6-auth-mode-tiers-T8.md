# Codex Execution Prompt — plan:m365-cps-trkB-p6-auth-mode-tiers:T8
...
- **Task:** `T8` — Pre-commit + commit
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p6-auth-mode-tiers-T8
...
Plan reference: `plan:m365-cps-trkB-p6-auth-mode-tiers:T8`. Branch: `feat/m365-cps-trkB`.
...
# 3. Validate
cd $M365_ROOT && pre-commit run --all-files && make test && git log --oneline -1 | grep 'plan:m365-cps-trkB-p6'
...
git add <files>
git commit -m "plan:m365-cps-trkB-p6-auth-mode-tiers:T8 — Pre-commit + commit"
```
...
- Validation command exits 0: `cd $M365_ROOT && pre-commit run --all-files && make test && git log --oneline -1 | grep 'plan:m365-cps-trkB-p6'`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p6-auth-mode-tiers:T8`
...
- **Validation result:** `cd $M365_ROOT && pre-commit run --all-files && make test && git log --oneline -1 | grep 'plan:m365-cps-trkB-p6'` exits 0
...
- Reference `plan:m365-cps-trkB-p6-auth-mode-tiers:T8` in commit message
