# Codex Execution Prompt — plan:m365-cps-trkB-p2-calendar-reads:T2
...
- **Task:** `T2` — Add graph.calendar.list registry entry
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p2-calendar-reads-T2
...
- **Task name:** Add graph.calendar.list registry entry
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T1
...
Add ActionSpec for graph.calendar.list (workload exchange; auth_modes ['auth_code_pkce', 'device_code']; scopes ['Calendars.Read']; risk low; rw read) to READ_ONLY_REGISTRY in src/m365_runtime/graph/registry.py. Endpoint per parent plan B-action spec table.

Plan reference: `plan:m365-cps-trkB-p2-calendar-reads:T2`. Branch: `feat/m365-cps-trkB`.
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import get_action; spec = get_action('graph.calendar.list'); assert spec.workload == 'exchange'; print('OK')"
...
git add <files>
git commit -m "plan:m365-cps-trkB-p2-calendar-reads:T2 — Add graph.calendar.list registry entry"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import get_action; spec = get_action('graph.calendar.list'); assert spec.workload == 'exchange'; print('OK')"`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p2-calendar-reads:T2`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import get_action; spec = get_action('graph.calendar.list'); assert spec.workload == 'exchange'; print('OK')"` exits 0
...
- Reference `plan:m365-cps-trkB-p2-calendar-reads:T2` in commit message
