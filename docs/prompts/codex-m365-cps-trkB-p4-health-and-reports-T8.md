# Codex Execution Prompt — plan:m365-cps-trkB-p4-health-and-reports:T8

## Governance Lock (Read FIRST)

- `AGENTS.md` / `CLAUDE.md` (M365 repo and UCP repo)
- `docs/NORTH_STAR.md` (UCP) / `Operations/NORTHSTAR.md` (M365)
- `plans/m365-cps-trkB-p4-health-and-reports/m365-cps-trkB-p4-health-and-reports.md` (parent phase plan)
...
- **Plan:** `plan:m365-cps-trkB-p4-health-and-reports`
- **Parent track:** `plan:m365-cps-trkB-coverage`
- **Master plan:** `plan:m365-capability-pack-surface-remediation`
- **Task:** `T8` — Add graph.reports.sharepoint_usage registry entry
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p4-health-and-reports-T8
...
- **Task name:** Add graph.reports.sharepoint_usage registry entry
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T7
...
Add ActionSpec for graph.reports.sharepoint_usage (workload reports; auth_modes ['app_only_secret', 'app_only_certificate']; scopes ['Reports.Read.All']; risk low; rw read) to READ_ONLY_REGISTRY in src/m365_runtime/graph/registry.py. Endpoint per parent plan B-action spec table.

Plan reference: `plan:m365-cps-trkB-p4-health-and-reports:T8`. Branch: `feat/m365-cps-trkB`.
...
- Cross-task changes (this prompt is scoped to T8 only)
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import get_action; spec = get_action('graph.reports.sharepoint_usage'); assert spec.workload == 'reports'; print('OK')"
...
git add <files>
git commit -m "plan:m365-cps-trkB-p4-health-and-reports:T8 — Add graph.reports.sharepoint_usage registry entry"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import get_action; spec = get_action('graph.reports.sharepoint_usage'); assert spec.workload == 'reports'; print('OK')"`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p4-health-and-reports:T8`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python -c "from m365_runtime.graph.registry import get_action; spec = get_action('graph.reports.sharepoint_usage'); assert spec.workload == 'reports'; print('OK')"` exits 0
...
- Reference `plan:m365-cps-trkB-p4-health-and-reports:T8` in commit message
