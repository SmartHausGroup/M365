# Codex Execution Prompt — plan:m365-cps-trkB-p5-directory-and-teams:T10
...
- **Task:** `T10` — UCP routing unit tests
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p5-directory-and-teams-T10
...
- **Task name:** UCP routing unit tests
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T9
...
Add UCP-side test_m365_integration entries that exercise execute_m365_action with the new actions through the alias table.

Plan reference: `plan:m365-cps-trkB-p5-directory-and-teams:T10`. Branch: `feat/m365-cps-trkB`.
...
# 3. Validate
cd $UCP_ROOT && .venv/bin/python -m pytest tests/test_m365_integration.py -k 'p5' -x
...
git add <files>
git commit -m "plan:m365-cps-trkB-p5-directory-and-teams:T10 — UCP routing unit tests"
```
...
- Validation command exits 0: `cd $UCP_ROOT && .venv/bin/python -m pytest tests/test_m365_integration.py -k 'p5' -x`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p5-directory-and-teams:T10`
...
- **Validation result:** `cd $UCP_ROOT && .venv/bin/python -m pytest tests/test_m365_integration.py -k 'p5' -x` exits 0
...
- Reference `plan:m365-cps-trkB-p5-directory-and-teams:T10` in commit message
