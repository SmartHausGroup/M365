# Codex Execution Prompt — plan:m365-cps-trkB-p6-auth-mode-tiers:T5
...
- **Task:** `T5` — Update admit() to enforce tier
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p6-auth-mode-tiers-T5
...
- **Task name:** Update admit() to enforce tier
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T4
...
admit() rejects with 'tier_insufficient' if session.tier < spec.min_tier.

Plan reference: `plan:m365-cps-trkB-p6-auth-mode-tiers:T5`. Branch: `feat/m365-cps-trkB`.
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python -m pytest tests/test_m365_admit_tier.py -x
...
git add <files>
git commit -m "plan:m365-cps-trkB-p6-auth-mode-tiers:T5 — Update admit() to enforce tier"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python -m pytest tests/test_m365_admit_tier.py -x`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p6-auth-mode-tiers:T5`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python -m pytest tests/test_m365_admit_tier.py -x` exits 0
...
- Reference `plan:m365-cps-trkB-p6-auth-mode-tiers:T5` in commit message
