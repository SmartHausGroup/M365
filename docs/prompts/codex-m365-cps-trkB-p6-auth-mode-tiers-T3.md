# Codex Execution Prompt — plan:m365-cps-trkB-p6-auth-mode-tiers:T3
...
- **Task:** `T3` — Update token store to record tier
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p6-auth-mode-tiers-T3
...
- **Task name:** Update token store to record tier
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T2
...
Token store schema gains tier field. Migration: existing tokens default to read-only.

Plan reference: `plan:m365-cps-trkB-p6-auth-mode-tiers:T3`. Branch: `feat/m365-cps-trkB`.
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python -m pytest tests/test_token_store_tier.py -x
...
git add <files>
git commit -m "plan:m365-cps-trkB-p6-auth-mode-tiers:T3 — Update token store to record tier"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python -m pytest tests/test_token_store_tier.py -x`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p6-auth-mode-tiers:T3`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python -m pytest tests/test_token_store_tier.py -x` exits 0
...
- Reference `plan:m365-cps-trkB-p6-auth-mode-tiers:T3` in commit message
