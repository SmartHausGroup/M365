# Codex Execution Prompt — plan:m365-cps-trkB-p6-auth-mode-tiers:T6
...
- **Task:** `T6` — Unit tests both repos
- **Branch:** `feat/m365-cps-trkB`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkB-p6-auth-mode-tiers-T6
...
- **Task name:** Unit tests both repos
- **Domain:** framework
- **Owner:** Phil Siniscalchi (Founder/Owner)
- **Dependencies:** T5
...
M365: test_m365_auth_tiers, test_m365_admit_tier. UCP: test_m365_auth_start_tier_passthrough.

Plan reference: `plan:m365-cps-trkB-p6-auth-mode-tiers:T6`. Branch: `feat/m365-cps-trkB`.
...
# 3. Validate
cd $M365_ROOT && .venv/bin/python -m pytest -k tier -x && cd $UCP_ROOT && .venv/bin/python -m pytest -k tier -x
...
git add <files>
git commit -m "plan:m365-cps-trkB-p6-auth-mode-tiers:T6 — Unit tests both repos"
```
...
- Validation command exits 0: `cd $M365_ROOT && .venv/bin/python -m pytest -k tier -x && cd $UCP_ROOT && .venv/bin/python -m pytest -k tier -x`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkB-p6-auth-mode-tiers:T6`
...
- **Validation result:** `cd $M365_ROOT && .venv/bin/python -m pytest -k tier -x && cd $UCP_ROOT && .venv/bin/python -m pytest -k tier -x` exits 0
...
- Reference `plan:m365-cps-trkB-p6-auth-mode-tiers:T6` in commit message
