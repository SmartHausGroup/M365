# Codex Execution Prompt — plan:m365-cps-trkC-p4-operator-capability-map:T5
...
- **Task:** `T5` — Pre-commit + commit
- **Branch:** `feat/m365-cps-trkC`

## Prompt Run Metadata

- Prompt version: v1.0
- Task ID: m365-cps-trkC-p4-operator-capability-map-T5
...
Plan reference: `plan:m365-cps-trkC-p4-operator-capability-map:T5`. Branch: `feat/m365-cps-trkC`.
...
# 3. Validate
cd $M365_ROOT && pre-commit run --all-files && make test && cd $UCP_ROOT && pre-commit run --all-files && make test && git log --oneline -1 | grep 'plan:m365-cps-trkC-p4'
...
git add <files>
git commit -m "plan:m365-cps-trkC-p4-operator-capability-map:T5 — Pre-commit + commit"
```
...
- Validation command exits 0: `cd $M365_ROOT && pre-commit run --all-files && make test && cd $UCP_ROOT && pre-commit run --all-files && make test && git log --oneline -1 | grep 'plan:m365-cps-trkC-p4'`
...
- [ ] Pre-commit green
- [ ] Commit message references `plan:m365-cps-trkC-p4-operator-capability-map:T5`
...
- **Validation result:** `cd $M365_ROOT && pre-commit run --all-files && make test && cd $UCP_ROOT && pre-commit run --all-files && make test && git log --oneline -1 | grep 'plan:m365-cps-trkC-p4'` exits 0
...
- Reference `plan:m365-cps-trkC-p4-operator-capability-map:T5` in commit message
