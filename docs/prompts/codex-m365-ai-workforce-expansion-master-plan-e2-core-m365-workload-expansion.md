# MATHS Prompt: E2 Core M365 Workload Expansion
...
- `PLAN_REF_ACK: plan:m365-ai-workforce-expansion-master-plan:E2`
- `NORTH_STAR_ACK: Operations/NORTHSTAR.md`
- `RULES_ACK: .cursor/rules/**/*.mdc`

## Execution Rules

- Run checks `M365-WORKFORCE-E2-C0` -> `M365-WORKFORCE-E2-C10` in strict order.
- Stop on first `FAIL` or `BLOCKED`.
- Output each check exactly:
  - `CHECK:<id> STATUS:<PASS|FAIL|BLOCKED> EVIDENCE:<path/command> NOTE:<one line>`
- Final outputs exactly:
  - `GATE:M365-WORKFORCE-E2 STATUS:<GO|NO-GO>`
  - `FINAL_DECISION:<GO|NO-GO>`

## Prompt Run Metadata

- Prompt version: `1.0`
- Task ID: `M365-WORKFORCE-E2`
- Run ID: `core-m365-workload-expansion`
...
- Plan refs in scope:
  ...
  - `plan:m365-ai-workforce-expansion-master-plan:E2`
...
- Task name: `Core M365 Workload Expansion`
- Domain: `runtime`
- Dependencies: `E1E`
- Allowlist:
...
- Success criteria:
  - `E2` produces its declared artifacts or runtime changes without scope drift.
  - `E2` updates plan and tracker state so the next dependency edge is explicit.
  - `E2` leaves deterministic evidence paths and validation commands for replay.
- Out of scope:
...
- Artifact/schema evidence:
  - `Artifacts created or updated for E2 exist with required metadata, status, and dependency fields.`
- Runtime/test evidence:
  - `Act-scoped validation commands for E2 are recorded with expected pass/fail semantics.`
- Governance evidence:
...
- Determinism evidence:
  - `A fixed-state replay of the act leaves the same plan status, prompt inventory, and validation signatures for E2.`

## T - Tie

- Dependency ties:
  - `E1E -> E2 may not claim GO unless predecessors are satisfied.`
  - `E2 -> downstream acts inherit its outputs and must stay blocked if E2 is NO-GO.`
- Known failure modes:
...
- GO criteria:
  - `All M365-WORKFORCE-E2-C0 through M365-WORKFORCE-E2-C10 checks pass with required evidence.`
- NO-GO criteria:
  - `Any missing required artifact, failed hard gate, unresolved blocker, or replay mismatch.`

## H - Harness (ordered checks)

`M365-WORKFORCE-E2-C0` Preflight

- Verify governance docs, plan refs, and prerequisite phase status.

`M365-WORKFORCE-E2-C1` Baseline inventory

- Capture the current baseline for E2, including artifact presence, phase status, and dependency state.

`M365-WORKFORCE-E2-C2` Intent and boundary lock

- Confirm the exact objective, success criteria, and out-of-scope boundary for E2.

`M365-WORKFORCE-E2-C3` Implementation or specification step 1

- Apply the primary deterministic change required for E2.

`M365-WORKFORCE-E2-C4` Implementation or specification step 2

- Apply the secondary deterministic change required for E2.

`M365-WORKFORCE-E2-C5` Schema and contract validation

- Hard-fail on missing required keys, statuses, links, or metadata.

`M365-WORKFORCE-E2-C6` Targeted validations

- Execute the act-scoped validation commands for E2.

`M365-WORKFORCE-E2-C7` Strict artifact validation

- Verify required paths exist and reflect the intended state only.

`M365-WORKFORCE-E2-C8` Deterministic replay

- Repeat the fixed-state validation and require consistent outputs.

`M365-WORKFORCE-E2-C9` Hard gates (strict order)

1. `Artifact presence and schema checks`
2. `Targeted act validation command(s)`
3. `git diff --check or approved equivalent`

`M365-WORKFORCE-E2-C10` Governance synchronization and final decision
...
- Replay checks:
  - `The plan status, prompt inventory, and act-level validation outputs for E2 must remain stable across repeated runs on fixed repo state.`

## Output Contract

- Deliverables:
  - `E2` artifacts and synchronized tracker state
  - `E2` validation evidence
- Validation results:
  - `M365-WORKFORCE-E2-C0..C10 statuses`
- Evidence links:
