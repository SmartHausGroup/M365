...
| **action** | `create_list` |
| **Mutating** | Yes |
| **Preconditions** | `params`: `siteId` or `site_id` (string, required), `displayName` or `display_name` or `name` or `title` (string, required). Optional `columns` list of column-definition objects. Mutations must be enabled. |
| **Result shape** \(\mathcal{S}_{\texttt{create\_list}}\) | `{ "list": object, "status": "created" }` |
| **Error cases** | Missing site identifier; missing display name; invalid `columns`; Graph not configured; mutations disabled. |
...
### 73A. provision_team_status_workflow

| Field | Specification |
|-------|----------------|
| **action** | `provision_team_status_workflow` |
| **Mutating** | Yes |
| **Preconditions** | `params`: `siteId` or `site_id` (string, required), organizer user context (`organizerUserId` / `organizerUserPrincipalName` / `userId` / `userPrincipalName` / `mailbox`, required), `recipients` (non-empty list or comma-delimited string), `meetingStart`, and `meetingEnd`. Optional: `meetingSubject`, `workflowName`, `trackerListName`, `timeZone`, and reminder/digest schedule overrides. Mutations must be enabled. |
| **Result shape** \(\mathcal{S}_{\texttt{provision\_team\_status\_workflow}}\) | `{ "status": "provisioned", "environment": object, "site": object, "tracker_list": object, "meeting": object, "reminder_flow": object, "digest_flow": object }` |
| **Error cases** | Missing required workflow inputs; no visible Power Platform environment; no reusable SharePoint/Outlook connection references; Graph or Power Automate provisioning failure; mutations disabled. |

### 73B. provision_incident_response_war_room

| Field | Specification |
|-------|----------------|
| **action** | `provision_incident_response_war_room` |
| **Mutating** | Yes |
| **Preconditions** | `params`: `incidentName`, `teamName`, `siteName`, and `incidentLeadUpn` (strings, required). Optional: `mailNickname`, `commandChannelName`, `runbookPath`, `planTitle`, `bucketName`, `seedTaskTitle`, `seedTaskDescription`, `activationRecipients`, `activationSenderUserPrincipalName`, `activationSubject`, `sendActivationMail`, and `forceSendActivation`. If `teamName` and `siteName` resolve to different workspace identities, `mailNickname` is required. `runbookPath` is bounded to a top-level `.docx` filename. Mutations must be enabled. |
| **Result shape** \(\mathcal{S}_{\texttt{provision\_incident\_response\_war\_room}}\) | `{ "status": "provisioned", "workspace": object, "team": object, "command_channel": object, "site": object, "runbook_document": object, "plan": object, "bucket": object, "seed_task": object, "activation_mail": object }` |
| **Error cases** | Missing required workflow inputs; mismatched workspace identity without explicit `mailNickname`; site or team readiness failure; Planner plan/bucket/task provisioning failure; mutations disabled. |
...
### 121A. provision_team_status_workflow

| Field | Specification |
|-------|----------------|
| **action** | `provision_team_status_workflow` |
| **Mutating** | Yes |
| **Preconditions** | Same as section 73A. This composite convenience action is bounded to one recurring meeting, one SharePoint tracker list, one reminder flow, and one digest flow. |
| **Result shape** \(\mathcal{S}_{\texttt{provision\_team\_status\_workflow}}\) | `{ "status": "provisioned", "environment": object, "site": object, "tracker_list": object, "meeting": object, "reminder_flow": object, "digest_flow": object }` |
| **Error cases** | Same as section 73A. |

### 121B. provision_incident_response_war_room

| Field | Specification |
|-------|----------------|
| **action** | `provision_incident_response_war_room` |
| **Mutating** | Yes |
| **Preconditions** | Same as section 73B. This composite convenience action is bounded to one shared group-backed incident workspace, one command channel, one runbook document, one Planner plan, one Planner bucket, one seed task, and one activation mail decision. |
| **Result shape** \(\mathcal{S}_{\texttt{provision\_incident\_response\_war\_room}}\) | `{ "status": "provisioned", "workspace": object, "team": object, "command_channel": object, "site": object, "runbook_document": object, "plan": object, "bucket": object, "seed_task": object, "activation_mail": object }` |
| **Error cases** | Same as section 73B. |
