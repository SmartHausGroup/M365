...
This document closes the M365-side operational scope of the standalone Graph runtime Integration Pack program. The active artifact is `com.smarthaus.m365@1.1.1`; the historical `1.1.0` artifact was over-claimed and is recorded as fix-required in `plans/m365-standalone-graph-runtime-integration-pack-fix/`. Live UCP through-the-installed-pack acceptance and any roadmap items beyond read-only Graph remain governed by their respective sibling plans.
